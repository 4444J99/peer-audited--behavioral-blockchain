...
| `https://<api-render-host>/health`                                         | unknown | **gap to `ship-soon`** — API health check is `/health` (per `render.yaml` `healthCheckPath` and `apps/api/src/modules/health/health.controller.ts` `@Controller('health')`); Render deploy pending |
| `https://<api-render-host>/api/docs`                                       | unknown | **gap** — Swagger UI exists in non-production only (`apps/api/src/main.ts:96`); production deploy will need a docs-route decision                                                                  |
