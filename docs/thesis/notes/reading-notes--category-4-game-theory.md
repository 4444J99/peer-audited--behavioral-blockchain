...
1. **Setup:** Define the Fury accuracy game: auditor _v_ submits verdict _r_ ∈ {PASS, FAIL} for proof _p_ with true state _s_ ∈ {PASS, FAIL}
...
3. **Dominance argument:** Show that for any prior belief _q_ = P(s = FAIL), the expected payoff of truthful reporting exceeds dishonest reporting
4. **Bound derivation:** Solve FA ≥ 0.8 for maximum tolerable false accusation ratio: _ā_/_n_ ≤ 0.05 (5%)
