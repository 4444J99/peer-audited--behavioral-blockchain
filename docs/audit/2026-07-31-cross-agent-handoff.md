...
- **Line numbers: grep the symbol, don't trust the number.** Revised 2026-07-31
  ...
  auditing, _before_ PR #858 landed — and #858 shifted eleven of them (e.g.
...
Not bugs — _false green_. Everything in PR #858 passed CI, had no TODO, no skipped
test, and was wrong:
...
- `render.yaml` failed _open_ on unresolvable location, defeating the US-only
  boundary.
...
_before_ a control was switched off — while still missing `POST /contracts`,
...
- Dockerfiles: `.config/docker/Dockerfile`, `src/api/Dockerfile`,
  ...
  26-vs-20 split a _live, untested six-major divergence today_, not a risk the
...
`^29.1.1` because 30 _breaks_ on Node 20 (`webidl.util.markAsUncloneable is not a
...
doing nothing** — `k6 run` sits inside the _same_ `try` as `k6 version`, so once
