# Engagement Economics — $100/hr Audience Build

Instantiation of `playbook--audience-growth-engine.md` §6 for the Jessica ↔ technical-partner
arrangement. Makes the paid engagement explicit so it stays a good deal for both sides and
doesn't blur into the equity partnership.

## Terms

- **Builder:** technical partner (audience-growth system + execution).
- **Payer / audience owner:** Jessica.
- **Rate:** $100/hr.
- **Weekly hour budget:** to be set with Jessica (recommend a fixed cap, e.g. 5–10 hrs/wk, so
  spend is predictable). Log every hour.
...
- Borrowed-audience + intermediary outreach setup and tracking.
...
> This boundary should be confirmed by both partners in writing before billing starts.
...
2. **Qualified Styx pipeline** — warm, high-intent waitlist + Cohort 1 (value to the company
   she's a 50% owner of).
3. **A reusable system** — the engine + filled templates, which she co-owns and which the next
   venture re-runs (compounding leverage, not a one-off spend).

A simple way to judge ROI weekly: cost = hours × $100; return = (net new followers + net new
email subscribers + net qualified waitlist) — tracked in the metrics doc. If a channel's
cost-per-result is out of line, reallocate (mirrors REGE CRIT:channel-efficiency).
