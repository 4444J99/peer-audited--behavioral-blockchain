...
- local web app via `npm run dev` in `apps/web`
- local API boot via `npm start` in `apps/api`
...
|---|---|---|---|---|---|---|
| `B7-01` | `High` | API bootstrap | API cannot start because `ComplianceModule` does not provide `TruthLogService` for `MedicalExemptionService`. | `cd apps/api && npm start` | Blocks meaningful end-to-end auth, contract, and readiness validation against a local backend. | [compliance.module.ts](/Users/4jp/Workspace/organvm-iii-ergon/peer-audited--behavioral-blockchain/apps/api/src/modules/compliance/compliance.module.ts#L16), [medical-exemption.service.ts](/Users/4jp/Workspace/organvm-iii-ergon/peer-audited--behavioral-blockchain/apps/api/src/modules/compliance/medical-exemption.service.ts#L17) |
| `B7-02` | `Medium` | Web auth UX | Login failure state surfaces raw transport error text instead of a product-safe degraded-state message. | Open `/login`, enter credentials, click `SIGN IN` while API is unavailable. | Poor tester UX and avoidable reviewer confusion if a backend dependency is down during review. | [login/page.tsx](/Users/4jp/Workspace/organvm-iii-ergon/peer-audited--behavioral-blockchain/apps/web/app/login/page.tsx#L17), [b7-console.log](/Users/4jp/Workspace/organvm-iii-ergon/peer-audited--behavioral-blockchain/output/playwright/b7-console.log) |
| `B7-03` | `Medium` | Web auth UX | Register failure state also surfaces raw transport error text after a valid submission. | Open `/register`, fill a valid form, click `CREATE ACCOUNT` while API is unavailable. | Same degraded-state problem as login; new-user entry path does not fail gracefully. | [register/page.tsx](/Users/4jp/Workspace/organvm-iii-ergon/peer-audited--behavioral-blockchain/apps/web/app/register/page.tsx#L20), [b7-register-network-error.png](/Users/4jp/Workspace/organvm-iii-ergon/peer-audited--behavioral-blockchain/output/playwright/b7-register-network-error.png) |
| `B7-04` | `Medium` | Web auth bootstrap | Public/auth/legal routes unconditionally call `/users/me` on mount, producing noisy `ERR_CONNECTION_REFUSED` console errors when the API is offline. | Open `/`, `/login`, `/register`, or `/legal/responsible-use` with API unavailable and inspect console. | Increases noise, obscures real failures, and makes public pages look broken when the backend is down. | [AuthContext.tsx](/Users/4jp/Workspace/organvm-iii-ergon/peer-audited--behavioral-blockchain/apps/web/contexts/AuthContext.tsx#L43), [api-client.ts](/Users/4jp/Workspace/organvm-iii-ergon/peer-audited--behavioral-blockchain/apps/web/services/api-client.ts#L288), [b7-console.log](/Users/4jp/Workspace/organvm-iii-ergon/peer-audited--behavioral-blockchain/output/playwright/b7-console.log) |
| `B7-05` | `Medium` | Web marketing / scope lock | Homepage copy still markets stake, `Ex-Bounty`, and escrow language that exceeds the narrow Phase 1 beta framing. | Open `/` and compare live copy to the Phase 1 scope lock. | Residual claim drift on a public surface; can misframe the current beta to testers and reviewers. | [page.tsx](/Users/4jp/Workspace/organvm-iii-ergon/peer-audited--behavioral-blockchain/apps/web/app/page.tsx#L17), [planning--phase1-private-beta-scope.md](/Users/4jp/Workspace/organvm-iii-ergon/peer-audited--behavioral-blockchain/docs/planning/planning--phase1-private-beta-scope.md#L5) |
...
| `B7-07` | `Low` | Form polish | Register form password inputs emit missing `autocomplete` warnings. | Open `/register` and inspect console. | Non-blocking, but avoidable browser warning on the first-user path. | [register/page.tsx](/Users/4jp/Workspace/organvm-iii-ergon/peer-audited--behavioral-blockchain/apps/web/app/register/page.tsx#L92), [b7-console.log](/Users/4jp/Workspace/organvm-iii-ergon/peer-audited--behavioral-blockchain/output/playwright/b7-console.log) |
...
- `B7-06`: `apps/web/app/icon.svg` now exists, so the favicon `404` is removed.
