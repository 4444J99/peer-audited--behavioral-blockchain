...
export async function setupAuthenticatedMocks(page: Page) {
  ...
  await page.route("**/api/auth/refresh", (route) =>
    route.fulfill({
      ...
      body: JSON.stringify({
        userId: MOCK_USER.id,
        token: "jwt-e2e-refreshed-token",
      }), // allow-secret: static e2e mock fixture, not a real credential
    }),
  );
  ...
  await page.route("**/api/notifications/stream-ticket", (route) =>
    route.fulfill({
      ...
      contentType: "application/json",
      body: JSON.stringify({
        ticket: "e2e-notifications-stream-ticket",
        expiresInSeconds: 60,
      }),
    }),
  );
