...
async function dismissTourOverlay(page: Page) {
  const hideBtn = page.locator('[data-guided-tour="panel"] button', {
    hasText: /hide|close|dismiss|collapse/i,
  });
...
for (const vp of VIEWPORTS) {
  test.describe(`Demo readiness @ ${vp.name} (${vp.width}x${vp.height})`, () => {
    ...
    test("root page loads with STYX heading, Private Beta badge, and feature grid", async ({
      page,
    }) => {
      ...
      await expect(
        page.getByText("Private Beta — Test-Money Pilot"),
      ).toBeVisible();
      ...
      await expect(
        page.getByRole("heading", { name: "PEER AUDIT", exact: true }),
      ).toBeVisible();
    });

    /* 2. Tour page is reachable — STYX TOUR heading, truth labels, three commitment steps */
    test("tour page has STYX TOUR heading, truth labels, and three commitment steps", async ({
      page,
    }) => {
      ...
      const truthLabels = page.getByText(
        /Working today|Test-money beta|Future enterprise capability/,
      );
    ...
    test("circles page has heading and all 5 circle sections", async ({
      page,
    }) => {
      ...
      await expect(
        page.getByText("Circle Gamma — Proof Integrity"),
      ).toBeVisible();
      await expect(page.getByText("Circle Delta — Retention")).toBeVisible();
      await expect(
        page.getByText("Circle Omega — The Enterprise"),
      ).toBeVisible();
    });
    ...
    test("login page renders email, password, and submit button", async ({
      ...
      await expect(
        page.locator('#email, input[type="email"], input[name="email"]'),
      ).toBeVisible();
      await expect(
...
test.describe("Guided tour overlay", () => {
  ...
  test("guided tour panel renders on /tour when test-money mode is active", async ({
    page,
  }) => {
...
test.describe("Contract creation page", () => {
  ...
  test("contracts/new renders the form when authenticated", async ({
    ...
    await page.route("**/api/compliance/eligibility", (route) =>
      route.fulfill({
        ...
        body: JSON.stringify({
          ...
          actions: { canCreateContract: true },
        }),
      }),
    );
    await page.route("**/api/identity/oath", (route) =>
      route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify({
          oath: { pledgeCopy: "I commit to no-contact recovery." },
        }),
      }),
    );
