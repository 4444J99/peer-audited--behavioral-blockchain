...
describe("Practitioner console page", () => {
  ...
  it("renders the top risk factors sorted by weighted contribution", async () => {
    ...
    expect(
      screen.getByText("2 missed check-ins in the last 14 days"),
    ).toBeTruthy();
    expect(screen.getByText("50% of grace days consumed")).toBeTruthy();
    expect(
      screen.getByText("10% of activity between midnight and 4am"),
    ).toBeTruthy();
  ...
  it("renders empty states when the caseload is clear", async () => {
    ...
    expect(await screen.findByText(/No clients assigned/)).toBeTruthy();
  ...
  it("shows the practitioner sign-in gate on 401", async () => {
    ...
    expect(
      await screen.findByText("Practitioner sign-in required"),
    ).toBeTruthy();
  ...
  it("surfaces API errors from the dashboard endpoint", async () => {
    fetchMock.mockImplementation((url: string) => {
      if (url === DASHBOARD_URL) {
        return Promise.resolve(
          jsonResponse(500, { message: "risk engine offline" }),
        );
      }
    ...
    expect(
      await screen.findByText("API 500: risk engine offline"),
    ).toBeTruthy();
  });
