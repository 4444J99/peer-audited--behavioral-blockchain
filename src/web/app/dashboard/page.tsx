    ...
    <div className="min-h-screen bg-black text-white font-sans p-6 md:p-12">
      ...
      <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-16 border-b border-neutral-800 pb-6">
        ...
        <div className="flex flex-wrap items-center gap-2 md:gap-3">
          <Link
            href="/wallet"
            className="px-3 md:px-4 py-2 bg-neutral-900 rounded-full border border-neutral-800 text-xs md:text-sm font-bold text-neutral-400 hover:text-white transition-colors"
          >
            WALLET
          </Link>
          <Link
          ...
          <Link
            href="/settings"
            className="px-3 md:px-4 py-2 bg-neutral-900 rounded-full border border-neutral-800 text-xs md:text-sm font-bold text-neutral-400 hover:text-white transition-colors flex items-center gap-1"
          >
            <Settings size={14} />
          </Link>
          <Link
          ...
          {authUser?.role === "ADMIN" && (
            <Link
              href="/admin"
              className="px-3 md:px-4 py-2 bg-neutral-900 rounded-full border border-red-900/50 text-xs md:text-sm font-bold text-red-400 hover:text-red-300 transition-colors"
            >
              ADMIN
            </Link>
          )}
          ...
          <Link
            href="/profile"
            className="flex items-center gap-2 px-3 md:px-4 py-2 bg-neutral-900 rounded-full border border-neutral-800 hover:border-neutral-600 transition-colors"
          >
            <User size={16} className="text-neutral-400" />
            <span className="font-bold text-sm hidden sm:inline">
              {authUser?.email ?? balance?.email ?? "Unknown"}
            </span>
          </Link>
          <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
        ...
        <div className="lg:col-span-1 space-y-8">
          ...
          <div className="p-8 bg-neutral-900 border border-neutral-800 rounded-3xl relative overflow-hidden">
            ...
            <h2 className="text-neutral-500 font-bold tracking-widest text-sm mb-4">
              INTEGRITY SCORE
            </h2>
          ...
          <div className="p-8 bg-neutral-900 border border-red-900/50 rounded-3xl relative overflow-hidden">
            ...
            <h2 className="text-neutral-500 font-bold tracking-widest text-sm mb-4">
              CAPITAL AT RISK (TEST CREDITS)
            </h2>
            <div className="flex items-baseline gap-1 mb-2">
              <span className="text-2xl text-red-500 font-medium">TEST-$</span>
              <span className="text-5xl font-black text-white">
                {activeStake.toFixed(2)}
              </span>
            </div>
            <p className="text-neutral-500 text-sm mb-6">
              {activeCount} active beta contract{activeCount !== 1 ? "s" : ""}
            </p>
            <Link
              href="/contracts/new"
              className="block w-full py-4 bg-red-600 hover:bg-red-700 text-white font-black rounded-xl transition-colors text-center"
            >
              CREATE NEW CONTRACT
            </Link>
          </div>
              ...
              <div className="p-8 bg-neutral-900 border border-neutral-800 rounded-3xl">
                ...
                <div className="mb-6">
                  ...
                  <p className="text-3xl font-black text-white">
                    TEST-$
                    {(progress.protectedVaultBalanceCents / 100).toFixed(2)}
                  </p>
                </div>

                {progress.activeContracts.length === 0 ? (
                  <p className="text-neutral-500 text-sm">
                    No active oath in progress.
                  </p>
                ) : (
                    ...
                    {progress.activeContracts.map((c) => {
                      ...
                      const pct =
                        totalDays > 0
                          ? Math.min(
                              100,
                              Math.round((attestedDays / totalDays) * 100),
                            )
                          : 0;
                      return (
                        <div key={c.id}>
                          <div className="flex justify-between items-baseline mb-1">
                            <span className="text-sm font-bold">
                              {(c.oath_category || "CONTRACT").replace(
                                /_/g,
                                " ",
                              )}
                            </span>
          ...
          <div className="p-8 bg-neutral-900 border border-neutral-800 rounded-3xl">
            ...
            {transactions.length === 0 ? (
              <p className="text-neutral-500 text-center py-8">
                No ledger entries yet. Create a contract to begin.
              </p>
            ) : (
              <div className="space-y-4">
                {transactions.map((tx) => {
                  ...
                  return (
                    <div
                      key={tx.id}
                      className="flex items-center justify-between p-4 bg-black rounded-2xl border border-neutral-800"
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-full bg-neutral-900 flex items-center justify-center">
                          <Activity
                            size={16}
                            className={
                              amount < 0 ? "text-red-500" : "text-green-400"
                            }
                          />
                        </div>
                        <div>
                          ...
                          <p className="text-xs text-neutral-500">
                            {new Date(tx.timestamp).toLocaleDateString()} &bull;{" "}
                            {tx.id.slice(0, 8)}
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p
                          className={`font-black ${amount < 0 ? "text-red-500" : "text-green-500"}`}
                        >
                          {amount < 0 ? "-" : "+"}TEST-$
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        <span className="font-black text-white">
                          TEST-${Number(c.stake_amount).toFixed(2)}
                        </span>
                        <span
                          className={`text-xs font-bold px-3 py-1 rounded-full ${statusBg} ${statusColor}`}
                        >
                          {status}
                        </span>
                      </div>
