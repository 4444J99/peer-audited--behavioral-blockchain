...
export default function HRDashboard() {
  ...
  const openRiskExposure =
    (metrics?.failedContracts ?? 0) + (metrics?.activeContracts ?? 0);
  ...
  const applyEnterpriseSelection = () => {
    ...
    if (!/^[A-Za-z0-9-]{6,128}$/.test(trimmed)) {
      setError(
        "Enterprise ID format is invalid. Use letters, numbers, and hyphens only.",
      );
      return;
    }
    ...
    return (
      <div className="min-h-screen bg-[#050505] text-white flex items-center justify-center p-6">
        <div className="max-w-xl text-center space-y-4">
          ...
          <h1 className="text-xl font-bold tracking-wide uppercase text-amber-200">
            Internal Feature Disabled
          </h1>
          <p className="text-sm text-gray-400 leading-6">
            B2B/HR analytics is hidden in the Phase 1 private beta build. This
            route remains internal-only and should not be exposed to testers.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white p-12 font-sans">
      <div className="mx-auto max-w-4xl px-4 py-3 mb-6 rounded-xl border border-blue-700/40 bg-blue-950/40 text-blue-200 text-xs font-bold uppercase tracking-wider text-center">
        FUTURE PREVIEW — Enterprise analytics are a development preview.
        Individual data is structurally redacted in any production deployment.
      </div>
      <header className="mb-12 border-b border-gray-800 pb-6 flex justify-between items-end gap-6">
        <div>
          <h1 className="text-3xl font-black tracking-tighter uppercase text-gray-300">
            Styx Corporate
          </h1>
        ...
        <div className="text-right text-sm">
          <div className="text-green-500 font-bold mb-1">
            Enterprise: {activeEnterpriseId}
          </div>
          <div className="text-gray-500">
            Total Enrolled: {metrics?.totalEmployees ?? 0} Employees
          </div>
          <div className="text-gray-500">
            Avg Integrity Score: {metrics?.avgIntegrityScore ?? 0}
          </div>
        </div>
      </header>
      ...
      <div className="mt-12 text-xs text-gray-700 uppercase tracking-widest text-center">
        PII and specific employee performance metrics are structurally redacted
        by the Aegis protocol.
      </div>
    </div>
  );
}
