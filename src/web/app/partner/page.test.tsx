...
describe("PartnershipCard", () => {
  const render = (
    overrides: Partial<React.ComponentProps<typeof PartnershipCard>> = {},
  ) =>
    renderToStaticMarkup(
      <PartnershipCard
  ...
  it("surfaces the veto only on recovery contracts", () => {
    ...
    expect(
      render({
        partnership: { ...partnership, oath_category: "BIOLOGICAL_WEIGHT" },
      }),
...
describe("CheckInThread", () => {
  ...
  it("offers the compose box only when a check-in is still PENDING", () => {
    ...
    const done = renderToStaticMarkup(
      <CheckInThread
        checkIns={[
          { ...scheduled, status: "COMPLETED", message: "held the line" },
        ]}
        draft=""
        busy={false}
        onDraftChange={noop}
