...
function selectValidFile() {
  ...
  const file = new File(["passport-bytes"], "passport.png", {
    type: "image/png",
  });
...
describe("KYC page", () => {
  ...
  it("rejects an oversized document with a validation error and keeps the flow idle", async () => {
    ...
    expect(
      await screen.findByText("Document exceeds the 10 MB limit."),
    ).toBeTruthy();
    const begin = screen
      .getByText("Begin Verification")
  ...
  it("rejects an unsupported document type", async () => {
    ...
    const exe = new File(["x"], "malware.exe", {
    ...
    expect(
      await screen.findByText(
        "Unsupported document type. Use JPEG, PNG, WebP, or PDF.",
      ),
    ).toBeTruthy();
  });
  ...
  it("renders the hosted-provider link for Stripe Identity sessions", async () => {
    ...
    expect(anchor.getAttribute("href")).toBe(
      "https://verify.stripe.com/session/vs_live_123",
    );
