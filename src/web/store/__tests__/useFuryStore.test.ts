...
describe("useFuryStore Integration", () => {
  ...
  it("polls fury queue and sets assignments on success", async () => {
    ...
    (
      (apiClient as any).api.issueFuryStreamCookie as jest.Mock
  ...
  it("sets error state when polling fails", async () => {
    ((apiClient as any).api.getFuryAssignments as jest.Mock).mockRejectedValue(
    ...
    expect(useFuryStore.getState().error).toBe(
      "Connection to Panopticon stream lost.",
    );
  });
