...
const FRICTION_QUESTIONS: FrictionQuestion[] = [
  {
    id: "physical_effort",
    question: "How much physical effort does your habit require?",
    description: "1 = very easy, 5 = very hard",
  },
  {
    id: "setup_time",
    question: "How many minutes does it take to prepare?",
    description: "1 = ready instantly, 5 = takes 30+ min to start",
  },
  {
    id: "bad_habit_access",
    question: "How easily can you access the bad habit?",
    description: "1 = blocked completely, 5 = one tap away",
  },
  {
    id: "environment_triggers",
    question: "How often are you triggered by your environment?",
    description: "1 = never, 5 = constantly",
  },
  {
    id: "social_support",
    question: "How much social support do you have?",
    description: "1 = strong support, 5 = going it alone",
  },
];
  ...
  const submitAudit = async () => {
    ...
    try {
      ...
      setResult({
        ...
        maxScore: 25,
        riskLevel: "UNKNOWN",
        recommendations: ["Failed to submit audit."],
      });
    } finally {
      setSubmitting(false);
    }
      ...
      <div className="space-y-4 p-4 bg-gray-900 rounded-lg border border-gray-800">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-semibold">
            Friction Score: {result.totalScore}/{result.maxScore}
          </h3>
        </div>
        <div
    ...
    <div className="space-y-6 p-4 bg-gray-900 rounded-lg border border-gray-800">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-gray-500">
          Step {step + 1} of {FRICTION_QUESTIONS.length}
        </span>
      </div>
      <div className="w-full bg-gray-800 rounded-full h-2">
        <div
          ...
          style={{
            width: `${((step + 1) / FRICTION_QUESTIONS.length) * 100}%`,
          }}
        />
      </div>
      <div key={step} className="space-y-4">
        ...
        <p className="text-sm text-gray-400">
          {FRICTION_QUESTIONS[step].description}
        </p>
        <div className="flex gap-2">
          {[1, 2, 3, 4, 5].map((val) => (
            <button
              key={val}
              onClick={() => setAnswer(FRICTION_QUESTIONS[step].id, val)}
      ...
      <div className="flex justify-between">
        ...
        >
          {submitting
            ? "Submitting..."
            : step < FRICTION_QUESTIONS.length - 1
              ? "Next"
              : "Submit"}
