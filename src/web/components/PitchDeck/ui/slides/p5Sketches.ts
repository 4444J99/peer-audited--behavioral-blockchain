...
export const sketchSlide1 = (p: p5) => {
  ...
  p.setup = () => {
    p.createCanvas(
      p.windowWidth > 800 ? 800 : p.windowWidth,
      p.windowHeight > 600 ? 600 : p.windowHeight,
    );

    p.loadFont(
      "https://cdnjs.cloudflare.com/ajax/libs/topcoat/0.2.0/font/SourceCodePro-Bold.otf",
      (loadedFont) => {
        font = loadedFont;
        // p5.js textToPoints returns an array of generic objects with x/y/alpha, we map it.
        let pts = loadedFont.textToPoints(
          "STYX",
          p.width / 2 - 180,
          p.height / 2 + 50,
          120,
...
export const sketchSlide2 = (p: p5) => {
  ...
  p.draw = () => {
    ...
    if (points.length > 0) {
      ...
      for (let i = 0; i < 5; i++)
        p.point(current.x + p.random(-20, 20), current.y + p.random(0, 50));
    }
  };
  p.windowResized = () =>
    p.resizeCanvas(p.windowWidth > 800 ? 800 : p.windowWidth, 400);
};
...
export const sketchSlide4 = (p: p5) => {
  ...
  p.draw = () => {
    ...
    p.textSize(14);
    p.text(
      "Hover over User node to simulate Fraud Detection",
      cx,
      p.height - 40,
    );
...
export const sketchSlide5 = (p: p5) => {
  ...
  p.draw = () => {
    ...
    p.textAlign(p.CENTER, p.CENTER);
    p.textSize(18);
    p.text("THE AEGIS PROTOCOL", cx, cy);
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (dist > maxRadius + 20) {
        ...
        p.stroke(255);
        p.circle(
          cx + p.cos(a) * (maxRadius + 20),
          cy + p.sin(a) * (maxRadius + 20),
          5,
        );
      }
    }
...
export const sketchSlide6 = (p: p5) => {
  ...
  p.setup = () => {
    ...
    for (let i = 0; i < 100; i++)
      users.push({ x: p.random(0, 200), y: p.random(p.height) });
  };
  ...
  p.draw = () => {
    ...
    p.text("Phase 1: B2C Chaos", 20, p.height - 20);
    p.textAlign(p.RIGHT, p.BOTTOM);
    p.text("Phase 2: B2B Order", p.width - 20, p.height - 20);
...
export const sketchSlide8 = (p: p5) => {
  ...
  p.draw = () => {
    ...
    nodes.forEach((n) => {
      ...
      p.text(n.n, n.x, n.y);
      p.fill(20);
      p.stroke(251, 146, 60); // Reset for next loop
    });
  };
  p.windowResized = () =>
    p.resizeCanvas(
...
export const sketchSlide10 = (p: p5) => {
  ...
  p.draw = () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      let py =
        p.height - ((p.frameCount * (1 + i * 0.5)) % (p.height - targetY + 50));
      if (py > targetY) {
        p.fill(163, 230, 53);
        p.noStroke();
        p.circle(x, py, 10);
