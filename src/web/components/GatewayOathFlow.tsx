  ...
  const checkEligibility = async () => {
    ...
    try {
      const res = await fetch(
        `/api/behavioral/gateway-oath/eligibility?stakeCents=${stakeCents}&durationDays=${durationDays}`,
      );
    ...
    <div className="space-y-6 p-4 bg-gray-900 rounded-lg border border-gray-800">
      ...
      {step === 2 && (
        <div className="space-y-3">
          ...
          {eligibility && (
            <div
              className={`p-3 rounded-lg ${eligibility.allowed ? "bg-green-900/30 text-green-300" : "bg-red-900/30 text-red-300"}`}
            >
              {eligibility.allowed
                ? "You are eligible for a Gateway Oath!"
                : eligibility.reason}
            </div>
          )}
        </div>
      ...
      <div className="flex justify-between">
        ...
        <button
          disabled={
            checking || (step === 2 && eligibility && !eligibility.allowed)
          }
          onClick={handleNext}
