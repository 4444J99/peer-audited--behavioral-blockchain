  ...
  windows: [
    {
      type: "DAY_21",
      severity: "CRITICAL",
      message: "Day 21 is the extinction burst.",
    },
    {
      type: "LATE_NIGHT",
      severity: "MEDIUM",
      message: "It is past midnight in your timezone.",
    },
  ],
  recommendations: [
    {
...
describe("DangerZoneBanners", () => {
  ...
  it("renders nothing when the contract is not in a danger zone", () => {
    const html = renderToStaticMarkup(
      <DangerZoneBanners
        status={{ ...IN_DANGER, inDangerZone: false, windows: [] }}
      />,
    );
...
describe("DangerZoneBanner", () => {
  it("renders nothing before the account-wide status resolves", () => {
    const html = renderToStaticMarkup(
      <DangerZoneBanner contractId="contract-abc-123" />,
    );
