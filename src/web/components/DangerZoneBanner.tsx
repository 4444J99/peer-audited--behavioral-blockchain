...
> = {
  LOW: {
    box: "bg-neutral-900 border-neutral-700",
    text: "text-neutral-300",
    label: "Low",
  },
  MEDIUM: {
    box: "bg-yellow-900/20 border-yellow-800",
    text: "text-yellow-400",
    label: "Medium",
  },
  HIGH: {
    box: "bg-orange-900/20 border-orange-800",
    text: "text-orange-400",
    label: "High",
  },
  CRITICAL: {
    box: "bg-red-900/20 border-red-800",
    text: "text-red-400",
    label: "Critical",
  },
};
 ...
 */
export function DangerZoneBanners({
  status,
}: {
  status: ContractDangerStatus | null;
}) {
  if (!status || !status.inDangerZone || status.windows.length === 0)
    return null;
    ...
    <div className="space-y-3">
      {status.windows.map((danger) => {
        ...
        const recommendation = status.recommendations.find(
          (r) => r.type === danger.type,
        );
        return (
          <div
            key={danger.type}
            className={`p-4 border rounded-xl ${style.box}`}
          >
 ...
 */
export default function DangerZoneBanner({
  contractId,
}: {
  contractId: string;
}) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (cancelled) return;
        setStatus(
          data.contracts.find((c) => c.contractId === contractId) ?? null,
        );
      } catch {
