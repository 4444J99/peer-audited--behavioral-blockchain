...
import RecoveryLockCountdown, {
  RecoveryLockPanel,
  formatCountdown,
} from "./RecoveryLockCountdown";
import type { RecoveryBreakRequest } from "../services/api-client";

const NOW = Date.parse("2026-02-01T00:00:00Z");

function makeRequest(
  overrides: Partial<RecoveryBreakRequest> = {},
): RecoveryBreakRequest {
  return {
    id: "break-1",
...
describe("formatCountdown", () => {
  it("renders hours, zero-padded minutes and seconds", () => {
    expect(formatCountdown(3 * 3600_000 + 7 * 60_000 + 5_000)).toBe(
      "3h 07m 05s",
    );
  });
...
describe("RecoveryLockPanel", () => {
  it("offers the break request form when nothing is queued", () => {
    const html = renderToStaticMarkup(
      <RecoveryLockPanel {...NOOP_PROPS} request={null} />,
    );
  ...
  it("shows the unlocked state for an UNLOCKED request even at a stale clock", () => {
    const html = renderToStaticMarkup(
      <RecoveryLockPanel
        {...NOOP_PROPS}
        request={makeRequest({ status: "UNLOCKED" })}
        now={NOW}
      />,
    );
