...
async function loadFixture(persona: string): Promise<Fixture> {
  ...
  const pending = fetch(`/demo-snapshot/${persona}.json`, {
    cache: "force-cache",
  })
    .then((response) => {
      if (!response.ok)
        throw new Error(`fixture ${persona}: HTTP ${response.status}`);
 ...
 */
export async function snapshotRespond<T>(
  path: string,
  method: string,
  ...
  if (verb === "POST" && path === "/auth/login") {
    ...
    return {
      ok: true,
      data: { token: "snapshot-session", userId: getPersona() } as T,
    }; // allow-secret: placeholder session marker
  }
  ...
  try {
    ...
    return {
      ...
      status: 503,
      message:
        "The demo snapshot data could not be loaded. Reload the page, or ask for the live walkthrough.",
    };
  }
