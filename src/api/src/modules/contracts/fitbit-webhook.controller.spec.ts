...
describe("FitbitWebhookController", () => {
  ...
  let fitbitService: {
    processReadinessState: jest.Mock;
    processSleepData: jest.Mock;
  };
  let fitbitSync: {
    resolveUserByFitbitId: jest.Mock;
    fetchDailySummary: jest.Mock;
  };
  ...
  const originalEnv = {
    ...
    FITBIT_SUBSCRIBER_VERIFICATION_CODE:
      process.env.FITBIT_SUBSCRIBER_VERIFICATION_CODE,
  };
  ...
  it("marks both webhook handlers @Public (signature auth, not session auth)", () => {
    expect(
      Reflect.getMetadata(
        IS_PUBLIC_KEY,
        FitbitWebhookController.prototype.handleNotifications,
      ),
    ).toBe(true);
    expect(
      Reflect.getMetadata(
        IS_PUBLIC_KEY,
        FitbitWebhookController.prototype.verifySubscriber,
      ),
    ).toBe(true);
  });
  ...
  describe("handleNotifications — signature verification", () => {
    ...
    it("rejects an unsigned request with 401 and audit-logs it", async () => {
      ...
      await controller.handleNotifications(
        makeReq(notificationBody()) as any,
        res,
      );
    ...
    it("rejects a signature computed with a different secret", async () => {
      const body = notificationBody();
      const forged = createHmac("sha1", "attacker-secret&")
        .update(body)
        .digest("base64");
      const res = makeRes();
  ...
  describe("handleNotifications — fetch-on-notify flow", () => {
    it("fetches data server-side and credits READY state for recovery contracts", async () => {
      ...
      expect(fitbitSync.fetchDailySummary).toHaveBeenCalledWith(
        "user-1",
        "2026-07-29",
      );
