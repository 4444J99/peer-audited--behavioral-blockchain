...
export class FitbitWebhookController {
  ...
  @Public()
  @ApiOperation({
    summary: "Fitbit subscriber endpoint verification (verify= challenge)",
  })
  @ApiExcludeEndpoint()
  verifySubscriber(
  ...
  async handleNotifications(
    ...
    if (!clientSecret) {
      this.logger.error(
        "Fitbit webhook invoked before FITBIT_CLIENT_SECRET was configured",
      );
    ...
    const expected = createHmac("sha1", `${clientSecret}&`)
      ...
      .digest("base64");
    if (
      !this.constantTimeEquals(Buffer.from(signature), Buffer.from(expected))
    ) {
  ...
  private async processNotification(
    ...
    if (collectionType !== "sleep") {
      this.logger.debug(
        `Fitbit collection '${collectionType}' not handled; skipping`,
      );
      return;
    }
    ...
    if (!sleep) {
      await this.truthLog.appendEvent(
        summary.manualSleepOnly
          ? "FITBIT_MANUAL_SLEEP_REJECTED"
          : "FITBIT_NO_DEVICE_DATA",
        {
          userId,
          fitbitUserId: ownerId,
          date,
    ...
    const recoveryContracts = contracts.rows.filter(
      (row: { oath_category?: string }) =>
