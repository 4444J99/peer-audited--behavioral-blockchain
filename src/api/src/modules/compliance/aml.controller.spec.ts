...
describe("AmlController", () => {
  ...
  beforeEach(() => {
    ...
    controller = new AmlController(
      amlScreening as unknown as AmlScreeningService,
    );
    jest.clearAllMocks();
  });
  ...
  describe("detectPatterns", () => {
    it("should run both detectors and the blocklist check with default window", async () => {
      const structuring = {
        ...
        severity: "HIGH",
        details:
          "4 transactions totaling $12000.00 within 24h, each below $3000.00",
      };
    ...
    it("should forward a custom windowHours to detectRapidMovement", async () => {
      ...
      expect(amlScreening.detectRapidMovement).toHaveBeenCalledWith(
        "user-2",
        72,
      );
    });
