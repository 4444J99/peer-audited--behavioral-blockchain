...
import {
  ...
  ComplianceMode,
  ComplianceAction,
} from "./compliance-policy.service";
import {
...
describe("CompliancePolicyService", () => {
  ...
  let mockIdentityVerification: jest.Mocked<
    Pick<IdentityVerificationService, "getUserComplianceStatus">
  >;
    ...
    service = new CompliancePolicyService(
      {
        query: jest
          .fn()
          .mockResolvedValue({ rows: [{ date_of_birth: "1990-01-01" }] }),
      } as any,
  ...
  describe("onModuleInit (PRV16 startup warning)", () => {
    it("should warn loudly on startup when KYC enforcement is disabled", () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringMatching(/KYC enforcement is DISABLED/i),
      );
    });
    ...
    it("should log an ERROR when KYC is explicitly disabled in production", () => {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        expect.stringMatching(/PRODUCTION has KYC_ENFORCEMENT_ENABLED=false/i),
      );
    });
  });
  ...
  describe("canCreateContract", () => {
    it("should allow in TIER_1 jurisdiction", () => {
      const result = service.canCreateContract({
  ...
  describe("canSubmitProof", () => {
    it("should allow in TIER_1 jurisdiction", () => {
      const result = service.canSubmitProof({
        tier: JurisdictionTier.TIER_1,
        state: "TX",
      });
      expect(result.allowed).toBe(true);
  ...
  describe("canPurchaseTicket", () => {
    it("should allow in TIER_1 jurisdiction", () => {
      const result = service.canPurchaseTicket({
        tier: JurisdictionTier.TIER_1,
        state: "FL",
      });
      expect(result.allowed).toBe(true);
  ...
  describe("evaluateKycRequirement", () => {
    ...
    it("should allow TIER_1 stakes ($20 or less) without KYC even when enforcement enabled", async () => {
      ...
      expect(result.allowed).toBe(true);
      expect(
        mockIdentityVerification.getUserComplianceStatus,
    ...
    it("should block when identity verification service is missing", async () => {
      ...
      const result = await serviceNoVerification.evaluateKycRequirement(
        "user-1",
        50,
      );
      expect(result.allowed).toBe(false);
  ...
  describe("evaluateUserComplianceForRequest", () => {
    it("should return base policy denial if jurisdiction blocks", async () => {
      ...
      const result = await service.evaluateUserComplianceForRequest(
        req,
        "user-1",
      );
      expect(result.allowed).toBe(false);
    ...
    it("should block a monetized action when DOB is missing (age gate fails closed)", async () => {
      ...
      const result = await serviceNoDob.evaluateUserComplianceForRequest(
        req,
        "user-1",
      );
      expect(result.allowed).toBe(false);
    ...
    it("should block a monetized action for an under-18 user", async () => {
      ...
      const serviceUnderage = new CompliancePolicyService(
        {
          query: jest
            .fn()
            .mockResolvedValue({
              rows: [{ date_of_birth: `${thisYear - 16}-01-01` }],
            }),
        } as any,
        mockIdentityVerification as unknown as IdentityVerificationService,
      );
      ...
      const result = await serviceUnderage.evaluateUserComplianceForRequest(
        req,
        "user-1",
      );
      expect(result.allowed).toBe(false);
    ...
    it("should handle missing identityVerification service gracefully", async () => {
      ...
      const result =
        await serviceNoVerification.evaluateUserComplianceForRequest(
          req,
          "user-1",
        );
      expect(result.allowed).toBe(false);
