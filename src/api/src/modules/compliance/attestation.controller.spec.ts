...
describe("AttestationController", () => {
  ...
  describe("registerIosKey", () => {
    it("delegates to registerIosAttestedKey with the authenticated user id", async () => {
      ...
      (
        mockDeviceAttestation.registerIosAttestedKey as jest.Mock
      ).mockResolvedValue(expected);

      const result = await controller.registerIosKey(user, body);

      expect(result).toEqual(expected);
      expect(mockDeviceAttestation.registerIosAttestedKey).toHaveBeenCalledWith(
        "user-1",
        body,
      );
    });
    ...
    it("propagates BadRequestException from the service", async () => {
      ...
      await expect(
        controller.registerIosKey(user, {
          keyId: "",
          attestationObject: "",
          challenge: "",
        }),
  ...
  describe("verifyIosAssertion", () => {
    it("delegates to verifyiOSAttestation with the authenticated user id", async () => {
      ...
      (
        mockDeviceAttestation.verifyiOSAttestation as jest.Mock
      ).mockResolvedValue(expected);

      const result = await controller.verifyIosAssertion(user, body);

      expect(result).toEqual(expected);
      expect(mockDeviceAttestation.verifyiOSAttestation).toHaveBeenCalledWith(
        "user-1",
        body,
      );
    });
  ...
  describe("verifyAndroidVerdict", () => {
    it("delegates to verifyAndroidAttestation with the authenticated user id", async () => {
      ...
      (
        mockDeviceAttestation.verifyAndroidAttestation as jest.Mock
      ).mockResolvedValue(expected);
      ...
      expect(result).toEqual(expected);
      expect(
        mockDeviceAttestation.verifyAndroidAttestation,
      ).toHaveBeenCalledWith("user-1", body);
    });
  });

  describe("revokeKey", () => {
    it("revokes the key for the authenticated user", async () => {
      (mockDeviceAttestation.revokeKey as jest.Mock).mockResolvedValue(
        undefined,
      );
      ...
      expect(mockDeviceAttestation.revokeKey).toHaveBeenCalledWith(
        "user-1",
        "key-abc",
      );
    });
    ...
    it("throws BadRequestException when keyId is missing", async () => {
      await expect(controller.revokeKey(user, { keyId: "" })).rejects.toThrow(
        BadRequestException,
      );
