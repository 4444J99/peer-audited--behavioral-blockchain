...
describe("RetentionScheduler", () => {
  ...
  let danger: {
    ...
    getContractDayNumber: jest.Mock;
  };
  ...
  const claimCalls = () =>
    pool.query.mock.calls.filter(([sql]) =>
      String(sql).includes("INSERT INTO retention_notifications"),
    );
    ...
    it("continues sweeping after one contract fails", async () => {
      pool.query
        .mockResolvedValueOnce({
          rows: [
            contractRow,
            { contract_id: "c-2", user_id: "u-2", timezone: "UTC" },
          ],
        })
        .mockResolvedValueOnce({ rows: [{ id: "rn-1" }] });
  ...
  describe("escalateOverdueCheckIns", () => {
    ...
    it("marks overdue check-ins MISSED, escalates, and notifies the owner", async () => {
      ...
      partners.escalateMissedCheckIn.mockResolvedValueOnce({
        level: "STAKE_WARNING",
        message:
          "Stake warning issued: partner p-1 missed 2 consecutive check-ins on contract c-1",
      });
