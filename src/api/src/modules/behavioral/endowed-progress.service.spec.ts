...
describe("EndowedProgressService", () => {
  ...
  describe("getCurrentTier", () => {
    ...
    it("returns Momentum at 0.30", () => {
      expect(service.getCurrentTier(0.3).name).toBe("Momentum");
    });

    it("returns Resilience at 0.50", () => {
      expect(service.getCurrentTier(0.5).name).toBe("Resilience");
    });

    it("returns Mastery at 0.70", () => {
      expect(service.getCurrentTier(0.7).name).toBe("Mastery");
    });

    it("returns Transcendence at 0.90", () => {
      expect(service.getCurrentTier(0.9).name).toBe("Transcendence");
    });
  ...
  describe("applyDynamicDownscaling", () => {
    ...
    it("throws NotFoundException for missing contract", async () => {
      ...
      await expect(service.applyDynamicDownscaling("missing")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
