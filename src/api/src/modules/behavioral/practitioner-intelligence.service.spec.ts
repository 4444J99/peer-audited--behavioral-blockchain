...
describe("PractitionerIntelligenceService", () => {
  ...
  describe("analyzeJournalEntry", () => {
    it("detects rationalization markers", async () => {
      const alerts = await service.analyzeJournalEntry(
        "user-1",
        "I think just one more time wont hurt",
      );
    ...
    it("returns empty array for clean text", async () => {
      const alerts = await service.analyzeJournalEntry(
        "user-5",
        "Had a great day today, feeling motivated and strong",
      );
    ...
    it("generates unique alert IDs", async () => {
      const alerts1 = await service.analyzeJournalEntry(
        "u1",
        "just one more time",
      );
      const alerts2 = await service.analyzeJournalEntry(
        "u1",
        "just one more time",
      );
  ...
  describe("getPractitionerDashboard", () => {
    it("returns dashboard for each assigned client", async () => {
      ...
      const assignments = {
        rows: [
          { client_id: "c1", alias: "Alice" },
          { client_id: "c2", alias: "Bob" },
        ],
      };
      const riskSignals = [
        { rows: [{ missed: 0 }] },
      ...
      mockQuery.mockImplementation(() =>
        Promise.resolve(allMocks[callIndex++]),
      );
