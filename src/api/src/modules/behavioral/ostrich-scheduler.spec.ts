...
describe("OstrichScheduler", () => {
  ...
  beforeEach(() => {
    ...
    scheduler = new OstrichScheduler(
  ...
  describe("detectAndIntervene", () => {
    it("sends intervention for at-risk users", async () => {
      ...
      expect(mockNotifications.create.mock.calls[0][0].type).toBe(
        "OSTRICH_MEDIUM",
      );
      expect(mockNotifications.create.mock.calls[1][0].type).toBe(
        "OSTRICH_HIGH",
      );
    });
    ...
    it("handles missing notification service gracefully", async () => {
      const schedulerWithoutNotifications = new OstrichScheduler(
        mockDetection as any,
        undefined,
      );
      mockDetection.detectAtRiskUsers.mockResolvedValueOnce([
