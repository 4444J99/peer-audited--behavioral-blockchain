...
describe("OstrichDetectionService", () => {
  ...
  describe("detectAtRiskUsers", () => {
    ...
    it("classifies by days since last active correctly", async () => {
      ...
      expect(users.find((u) => u.userId === "user-med")?.riskLevel).toBe(
        "MEDIUM",
      );
      expect(users.find((u) => u.userId === "user-med2")?.riskLevel).toBe(
        "MEDIUM",
      );
      expect(users.find((u) => u.userId === "user-high")?.riskLevel).toBe(
        "HIGH",
      );
    });
  });
  ...
  describe("recordMissedProof", () => {
    it("increments consecutive_missed_proofs", async () => {
      ...
      expect(mockPool.query.mock.calls[0][0]).toContain(
        "consecutive_missed_proofs = consecutive_missed_proofs + 1",
      );
