  ...
  describe("calculateRelapseRiskScore", () => {
    ...
    it("returns HIGHER risk with strikes", async () => {
      ...
      pool.query
        ...
        .mockResolvedValueOnce({
          rows: [{ d: "2026-07-22" }, { d: "2026-07-21" }, { d: "2026-07-20" }],
          rowCount: 3,
        } as any)
    ...
    it("aggregates all fields", async () => {
      ...
      pool.query.mockImplementation(async (sql: string) => {
        if (sql.includes("SELECT DATE(created_at) AS proof_date")) {
          return {
            rows: [{ proof_date: "2026-07-22" }, { proof_date: "2026-07-21" }],
            rowCount: 2,
          };
        }
        ...
        if (
          sql.includes("SELECT COUNT(*) AS cnt FROM proofs") &&
          sql.includes("status = 'VERIFIED'") &&
      ...
      const summary = await service.getDashboardSummary("user-1", "contract-1");
