...
describe("DataLakeService", () => {
  ...
  describe("extractSnapshot", () => {
    it("should return a complete snapshot with all three sections", async () => {
      ...
      const result = await service.extractSnapshot(
        "ent-001",
        "2026-01-01",
        "2026-02-01",
      );
  ...
  describe("small-group suppression (PRV11)", () => {
    it("should suppress categories/months/cohorts below MIN_GROUP_SIZE", async () => {
        ...
        rows: [
          {
            ...
            completions: "1",
            failures: "0",
            avg_integrity_delta: "5",
          },
          {
            month: "2026-02",
            distinct_employees: "8",
            new_contracts: "12",
            completions: "9",
            failures: "1",
            avg_integrity_delta: "2",
          },
        ],
      });
      ...
      expect(result.contractMetrics.map((c) => c.oathCategory)).toEqual([
        "BIG",
      ]);
      expect(result.behavioralTrends.map((b) => b.month)).toEqual(["2026-02"]);
      expect(result.cohortAnalysis.map((c) => c.cohortMonth)).toEqual([
        "2025-12",
      ]);
    });
  });
  ...
  describe("setupReplicationSlot", () => {
    ...
    it("should propagate unexpected errors", async () => {
      ...
      await expect(service.setupReplicationSlot()).rejects.toThrow(
        "connection failed",
      );
    });
  });
