...
describe("HubSpotConnector", () => {
  ...
  describe("pushEmployeeEvent", () => {
    ...
    it("should search for contact and create a note", async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () =>
          Promise.resolve({ total: 1, results: [{ id: "hs-contact-1" }] }),
      });
    ...
    it("should throw on note creation failure", async () => {
      ...
      await expect(connector.pushEmployeeEvent(testEvent)).rejects.toThrow(
        "HubSpot note creation failed: 500",
      );
    });
  ...
  describe("syncUserList", () => {
    ...
    it("should filter by styx_enterprise_id", async () => {
      ...
      expect(body.filterGroups[0].filters[0].propertyName).toBe(
        "styx_enterprise_id",
      );
