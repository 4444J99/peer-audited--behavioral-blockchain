...
describe("SalesforceConnector", () => {
  ...
  describe("pushEmployeeEvent", () => {
    ...
    it("should throw when Salesforce is not configured (no base URL)", async () => {
      ...
      await expect(unconfigured.pushEmployeeEvent(testEvent)).rejects.toThrow(
    ...
    it("should throw on auth failure", async () => {
      ...
      await expect(connector.pushEmployeeEvent(testEvent)).rejects.toThrow(
        "Salesforce auth failed: 401",
      );
    });
    ...
    it("should re-authenticate once and retry on a 401 (expired token) (PRV17)", async () => {
      mockFetch
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve({ access_token: "stale_tok" }),
        }) // initial auth
        .mockResolvedValueOnce({ ok: false, status: 401 }) // push -> token expired
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve({ access_token: "fresh_tok" }),
        }) // re-auth
        .mockResolvedValueOnce({ ok: true }); // retried push succeeds

      await expect(
        connector.pushEmployeeEvent(testEvent),
    ...
    it("should propagate a 401 that persists after re-authentication (PRV17)", async () => {
      mockFetch
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve({ access_token: "tok1" }),
        }) // auth
        .mockResolvedValueOnce({ ok: false, status: 401 }) // push 401
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve({ access_token: "tok2" }),
        }) // re-auth
        .mockResolvedValueOnce({ ok: false, status: 401 }); // still 401

      await expect(connector.pushEmployeeEvent(testEvent)).rejects.toThrow(
        "Salesforce push failed: 401",
      );
    });
    ...
    it("should cache access token across calls", async () => {
      mockFetch
        .mockResolvedValueOnce({
          ok: true,
          json: () => Promise.resolve({ access_token: "cached_tok" }),
        })
  ...
  describe("syncUserList", () => {
    ...
    it("should throw on query failure", async () => {
      ...
      await expect(connector.syncUserList("ent-001")).rejects.toThrow(
    ...
    it("should throw when not configured", async () => {
      process.env.SALESFORCE_BASE_URL = "";
      const unconfigured = new SalesforceConnector();
      await expect(unconfigured.syncUserList("ent-001")).rejects.toThrow(
        "Salesforce not configured",
      );
    });
  });
