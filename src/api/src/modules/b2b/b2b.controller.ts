...
import {
  ...
  CreateCohortDto,
  UpdateCohortDto,
} from "./cohort-orchestration.service";
import {
...
export class B2BController {
   ...
   */
  private assertEmployeeEventType(
    value: string,
    field: string,
  ): EmployeeEventType {
  ...
  @ApiOperation({
    summary: "Register a webhook URL for enterprise event notifications",
  })
  ...
  @ApiOperation({
    summary: "List the active webhook subscriptions of an enterprise",
  })
  ...
  async testWebhook(
    ...
    await this.assertEnterpriseMembership(user.id, body.enterpriseId);
    const sent = await this.webhook.dispatchEnterpriseMetricEvent(body.url, {
      type: "TEST",
  ...
  @ApiOperation({
    summary: "Extract a time-bounded snapshot from the enterprise data lake",
  })
  ...
  @ApiOperation({
    summary: "Get the aggregate corporate integrity score for an enterprise",
  })
  ...
  @ApiOperation({
    summary:
      "Push an employee behavioral event to the configured CRM connectors",
  })
  async pushCrmEvent(
    @CurrentUser() user: { id: string },
    @Param("enterpriseId") enterpriseId: string,
  ...
  @ApiOperation({
    summary: "Log a CRM interaction against an enterprise employee",
  })
  async logCrmInteraction(
    @CurrentUser() user: { id: string },
    @Param("enterpriseId") enterpriseId: string,
    @Body()
    body: { email: string; type: string; metadata?: Record<string, unknown> },
  ) {
    await this.assertEnterpriseMembership(user.id, enterpriseId);
  ...
  @ApiOperation({
    summary: "Sync an enterprise employee into the configured CRM",
  })
  async syncCrmUser(
    @CurrentUser() user: { id: string },
    @Param("enterpriseId") enterpriseId: string,
  ...
  @ApiOperation({
    summary: "Create a new behavioral contract cohort for clients/employees",
  })
  async createCohort(
    @CurrentUser() user: { id: string },
    @Param("enterpriseId") enterpriseId: string,
  ...
  @ApiOperation({
    summary: "Batch invite participants to an enterprise cohort",
  })
  async inviteCohortParticipants(
    ...
    await this.assertEnterpriseMembership(user.id, enterpriseId);
    return this.cohorts.inviteParticipants(
      enterpriseId,
      cohortId,
      body.emails || [],
    );
  ...
  @ApiOperation({
    summary: "Get detailed compliance and progress metrics for a cohort",
  })
