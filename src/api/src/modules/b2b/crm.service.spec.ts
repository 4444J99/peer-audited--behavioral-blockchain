...
describe("CrmService", () => {
  ...
  let mockSalesforce: jest.Mocked<
    Pick<SalesforceConnector, "pushEmployeeEvent" | "syncUserList">
  >;
  let mockHubspot: jest.Mocked<
    Pick<HubSpotConnector, "pushEmployeeEvent" | "syncUserList">
  >;
  ...
  describe("pushEmployeeEvent", () => {
    ...
    it("should not throw when Salesforce is not configured", async () => {
      mockSalesforce.pushEmployeeEvent.mockRejectedValue(
        new Error("Salesforce not configured"),
      );
      mockHubspot.pushEmployeeEvent.mockResolvedValue(undefined);

      await expect(
        service.pushEmployeeEvent("ent-001", testEvent),
    ...
    it("should not throw when HubSpot is not configured", async () => {
      mockSalesforce.pushEmployeeEvent.mockResolvedValue(undefined);
      mockHubspot.pushEmployeeEvent.mockRejectedValue(
        new Error("HubSpot not configured"),
      );
    ...
    it("should handle non-config errors gracefully", async () => {
      mockSalesforce.pushEmployeeEvent.mockRejectedValue(
        new Error("Salesforce push failed: 500"),
      );
      mockHubspot.pushEmployeeEvent.mockResolvedValue(undefined);
