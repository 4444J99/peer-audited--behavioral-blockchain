...
describe("WebhookSubscriptionService", () => {
  ...
  beforeEach(() => {
    ...
    mockWebhook = {
      assertDeliverableUrl: jest.fn().mockResolvedValue(undefined),
    };
  ...
  describe("enqueueContractResolved", () => {
    it("should enqueue one job per active subscription", async () => {
      mockPool.query.mockResolvedValueOnce({
        rows: [
          row,
          { ...row, id: "sub-2", url: "https://hooks.example.com/second" },
        ],
      });
    ...
    it("should send the salted pseudonym, never the raw user id", async () => {
      ...
      expect(mockAnonymize.hashUserId).toHaveBeenCalledWith(
        "user-9",
        "ent-001",
      );
