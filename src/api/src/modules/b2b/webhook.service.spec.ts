...
describe("WebhookService", () => {
  ...
  describe("signature verification", () => {
    ...
    it("should reject a tampered payload", () => {
      ...
      expect(service.verify(timestamp, '{"event":"tampered"}', sig)).toBe(
        false,
      );
    });
    ...
    it("should reject an invalid signature", () => {
      expect(
        service.verify(currentTimestamp(), "{}", "invalid-signature"),
      ).toBe(false);
    });
  ...
  describe("delivery with retry", () => {
    ...
    it("should retry on 500 errors up to max retries", async () => {
      ...
      const promise = service.deliverWithRetry("https://example.com/webhook", {
        event: "test",
      });
  ...
  describe("SSRF guard (assertSafeWebhookUrl via deliverWithRetry)", () => {
    ...
    it("should reject literal loopback / private / link-local IPs", async () => {
      for (const url of [
        ...
        await expect(
          service.deliverWithRetry(url, { event: "x" }),
        ).rejects.toThrow();
      }
      expect(sendSpy).not.toHaveBeenCalled();
    });
    ...
    it("should reject when DNS resolves a public name to a private IP (rebinding)", async () => {
      ...
      await expect(
        service.deliverWithRetry("https://rebind.example.com/hook", {
          event: "x",
        }),
