...
export class ExpoPushProvider implements PushProvider {
  ...
  async send(message: PushMessage): Promise<PushResult> {
    if (!process.env.EXPO_ACCESS_TOKEN) {
      this.logger.warn(
        "EXPO_ACCESS_TOKEN not set — using dev fallback (no-op)",
      );
    ...
    try {
      ...
      switch (pushStatus) {
        ...
        case "error": {
          ...
          if (
            code === DEVICE_GONE_CODE ||
            pushMessage?.includes(DEVICE_GONE_CODE)
          ) {
  ...
  async fetchReceipts(ticketIds: string[]): Promise<Map<string, PushReceipt>> {
    ...
    if (!process.env.EXPO_ACCESS_TOKEN) {
      ...
      this.logger.warn(
        "EXPO_ACCESS_TOKEN not set — cannot fetch push receipts",
      );
      return receipts;
    }
    ...
    for (let i = 0; i < ticketIds.length; i += EXPO_RECEIPT_BATCH_SIZE) {
      ...
      try {
        ...
        if (result.errors?.length) {
          this.logger.error(
            `Expo getReceipts reported ${result.errors.length} request error(s): ${result.errors
