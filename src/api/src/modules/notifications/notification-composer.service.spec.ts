...
describe("NotificationComposerService", () => {
  ...
  describe("compose", () => {
    it("routes to composeCheckInReminder", () => {
      ...
      expect(result.body).toBe(
        "Time for your daily check-in. Your partner is waiting.",
      );
    });
    ...
    it("routes to composeStreakMilestone", () => {
      ...
      expect(result.body).toBe(
        "14 days strong. You're proving something to yourself.",
      );
    });
    ...
    it("routes to composeWeekendWarning", () => {
      ...
      expect(result.body).toBe(
        "Weekend ahead. Pre-commit your routine. We're here.",
      );
    });
    ...
    it("throws on unknown type", () => {
      expect(() =>
        service.compose({ type: "UNKNOWN" as any, userId: "u1" }),
  ...
  describe("metadata interpolation", () => {
    it("interpolates day in danger zone alert", () => {
      ...
      expect(result.body).toBe(
        "Day 5: The first 72 hours are the hardest. You're not alone.",
      );
    });
    ...
    it("defaults day to 1 when missing", () => {
      ...
      expect(result.body).toBe(
        "Day 1: The first 72 hours are the hardest. You're not alone.",
      );
    });
    ...
    it("interpolates streak count", () => {
      ...
      expect(result.body).toBe(
        "7 days strong. You're proving something to yourself.",
      );
    });
    ...
    it("defaults streak to 1", () => {
      ...
      expect(result.body).toBe(
        "1 days strong. You're proving something to yourself.",
      );
    });
  });
  ...
  describe("compose standalone methods", () => {
    ...
    it("composeMissedProof returns expected content", () => {
      ...
      expect(result.title).toBe("Missed Proof");
      expect(result.body).toBe(
        "You missed your proof window. Get back on track — one slip doesn't define you.",
      );
    ...
    it("composeCrisisResource returns expected content", () => {
      ...
      expect(result.title).toBe("Support Available");
      expect(result.body).toBe(
        "If you're in crisis, help is available 24/7. Tap for resources.",
      );
