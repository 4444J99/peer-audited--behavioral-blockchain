...
describe("EnforcementService", () => {
  ...
  beforeEach(() => {
    ...
    mockLedger = {
      recordTransaction: jest.fn().mockResolvedValue("txn-reversal-1"),
    };
  ...
  describe("confirmCase", () => {
    it("rejects when the pending case cannot be claimed", async () => {
      ...
      await expect(service.confirmCase("case-x")).rejects.toThrow(
        "Pending case not found",
      );
    });
    ...
    it("refuses an explicit non-positive amount on a financial penalty", async () => {
      ...
      await expect(
        service.confirmCase("case-1", "STAKE_SLASH", 0),
      ).rejects.toThrow(/requires a positive amountCents/);
    });
  ...
  describe("resolveAppeal", () => {
    ...
    it("resolves an appeal as REVERSED and REFUNDS the slashed money", async () => {
      ...
      const sqlIssued = mockPool.query.mock.calls.map((c: any[]) =>
        String(c[0]),
      );
      expect(
        sqlIssued.some((sql) => /DELETE FROM fury_penalties/.test(sql)),
      ).toBe(false);
      expect(
        sqlIssued.some((sql) => /reversal_transaction_id = \$2/.test(sql)),
      ).toBe(true);
      expect(mockTruthLog.appendEvent).toHaveBeenCalledWith(
        "FURY_PENALTY_REVERSED",
        expect.objectContaining({
          amountCents: 500,
          reversalTransactionId: "txn-reversal-1",
        }),
      );
    });
    ...
    it("throws when case is not in APPEALED status", async () => {
      ...
      await expect(service.resolveAppeal("case-1", "UPHELD")).rejects.toThrow(
        "Appealed case not found",
      );
    });
  });
