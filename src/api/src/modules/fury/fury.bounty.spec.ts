...
describe("FuryWorker — Bounty Economy", () => {
  ...
  const demotionStatsMock = {
    rows: [
      { total_audits: "5", successful_audits: "4", false_accusations: "0" },
    ],
  };
  ...
  function setupConsensusFlow(opts: {
    ...
    mockPool.query.mockResolvedValueOnce({
      rows: [
        {
          id: opts.proofId ?? "proof-1",
          user_id: "host-user",
          total_furies: votes.length,
    ...
    if (outcome !== "SPLIT") {
      ...
      mockPool.query.mockResolvedValueOnce({
        rows: [{ id: "escrow-account" }],
      });
      // SYSTEM_REVENUE lookup
      mockPool.query.mockResolvedValueOnce({
        rows: [{ id: "revenue-account" }],
      });
  ...
  it("should pay AUDITOR_STAKE_AMOUNT bounty to correct Fury voters", async () => {
    ...
    expect(mockLedger.recordTransaction).toHaveBeenCalledWith(
      ...
      undefined,
      "consensus:proof-bounty:fury-1:bounty",
    );
    expect(mockLedger.recordTransaction).toHaveBeenCalledWith(
      "escrow-account",
      "acct-f2",
      ...
      { type: "FURY_BOUNTY", consensusProofId: "proof-bounty" },
      undefined,
      "consensus:proof-bounty:fury-2:bounty",
    );
  ...
  it("should charge AUDITOR_STAKE_AMOUNT penalty for incorrect Fury votes", async () => {
    setupConsensusFlow({
      ...
      outcome: "VERIFIED",
      furyAccountIds: {
        "fury-right": "acct-right",
        "fury-wrong": "acct-wrong",
      },
    });
    ...
    expect(mockLedger.recordTransaction).toHaveBeenCalledWith(
      ...
      undefined,
      "consensus:proof-pen:fury-right:bounty",
    );
    ...
    expect(mockLedger.recordTransaction).toHaveBeenCalledWith(
      ...
      undefined,
      "consensus:proof-pen:fury-wrong:penalty",
    );

    expect(mockTruthLog.appendEvent).toHaveBeenCalledWith(
      "FURY_PENALTY_CHARGED",
      expect.objectContaining({
  ...
  it("should charge penalty for honeypot failures", async () => {
    ...
    expect(sql.some((s) => /INSERT INTO fury_enforcement_cases/.test(s))).toBe(
      true,
    );
  ...
  it("should skip financial bounty for Furies without account_id", async () => {
    ...
    expect(mockLedger.recordTransaction).toHaveBeenCalledWith(
      ...
      undefined,
      "consensus:proof-skip:fury-with-acct:bounty",
    );
  });
  ...
  it("LC5: should NOT re-apply scoring or re-disburse bounties when consensus side effects already exist (re-claim)", async () => {
    ...
    const scoreCalls = mockPool.query.mock.calls.filter(
      (c) =>
        typeof c[0] === "string" &&
        (c[0].includes("integrity_score + 2") ||
          c[0].includes("integrity_score - 5")),
    );
