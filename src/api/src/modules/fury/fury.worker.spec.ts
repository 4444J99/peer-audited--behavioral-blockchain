  ...
  describe("checkConsensus", () => {
    ...
    it("should apply fraud penalty to flagged Furies on honeypot detection", async () => {
      ...
      const penaltyCalls = mockPool.query.mock.calls.filter(
        (c) =>
          typeof c[0] === "string" && c[0].includes("integrity_score - 15"),
      );
        ...
        rows: [
          {
            is_honeypot: true,
            contract_id: "c-1",
            honeypot_expected_verdict: "PASS",
          },
        ],
      });
      (mockConsensus.evaluate as jest.Mock).mockResolvedValueOnce({
        ...
        rows: [
          {
            is_honeypot: false,
            contract_id: "c-real",
            honeypot_expected_verdict: null,
          },
        ],
      });
      (mockConsensus.evaluate as jest.Mock).mockResolvedValueOnce({
    ...
    it("should revert proof status to UNDER_REVIEW and rethrow if resolution throws after claim", async () => {
      ...
      (mockConsensus.evaluate as jest.Mock).mockRejectedValueOnce(
        new Error("engine exploded"),
      );
      // revert UPDATE
      mockPool.query.mockResolvedValueOnce({ rows: [] });

      await expect(worker.checkConsensus("proof-boom")).rejects.toThrow(
        "engine exploded",
      );

      // The last query must revert RESOLVING -> UNDER_REVIEW for this proof.
      const revertCall =
        mockPool.query.mock.calls[mockPool.query.mock.calls.length - 1];
      expect(revertCall[0]).toMatch(
        /UPDATE proofs SET status = 'UNDER_REVIEW'/,
      );
    ...
    it("should call resolveContract(COMPLETED) when consensus is VERIFIED and contract_id exists", async () => {
      ...
      // claim resolution
      mockPool.query.mockResolvedValueOnce({
        rows: [{ id: "proof-resolve-complete" }],
      });
      mockPool.query.mockResolvedValueOnce({
