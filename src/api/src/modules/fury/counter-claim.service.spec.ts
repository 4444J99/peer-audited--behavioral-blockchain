...
describe("CounterClaimService (Issue #81)", () => {
  ...
  it("files a counter-claim with $1.00 filing fee and logs event", async () => {
    const claim = await service.fileCounterClaim("user-1", {
      ...
      reason:
        "Auditor repeatedly left hostile comments rejecting valid GPS proof",
  ...
  it("substantiates claim, slashes auditor, and updates truth log", async () => {
    ...
    const resolution = await service.adjudicateCounterClaim(
      claim.id,
      "judge-42",
      {
        decision: "SUBSTANTIATED",
        judgeNotes:
          "Evidence confirms collusion; auditor confirmed acquaintance with subject",
  ...
  it("dismisses frivolous claim and forfeits user filing fee", async () => {
    const claim = await service.fileCounterClaim("user-1", {
      claimType: "BIAS",
      reason:
        "Auditor rejected my dark photo even though rules say good lighting required",
    });
    ...
    const resolution = await service.adjudicateCounterClaim(
      ...
      "judge-42",
      {
        ...
        judgeNotes:
          "Photo was completely black, auditor followed guideline correctly",
      },
    );
