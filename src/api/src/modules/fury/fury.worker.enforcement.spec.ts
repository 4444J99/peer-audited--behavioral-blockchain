...
describe("FuryWorker — honeypot enforcement auto-open", () => {
  ...
  beforeEach(() => {
    ...
    mockEnforcement = {
      evaluateCollusion: jest.fn().mockResolvedValue(undefined),
    };
  ...
  it("does not strand the proof in RESOLVING when case filing throws", async () => {
    ...
    mockEnforcement.evaluateCollusion.mockRejectedValueOnce(
      new Error("pg down"),
    );
    const errorSpy = jest
    ...
    await expect(
      worker.checkConsensus("proof-hp-boom"),
    ...
    const revertCalls = mockPool.query.mock.calls.filter(
      (c) =>
        typeof c[0] === "string" &&
        c[0].includes("SET status = 'UNDER_REVIEW'"),
    );
  ...
  it("resolves consensus normally when no EnforcementService is wired", async () => {
    ...
    await expect(
      bareWorker.checkConsensus("proof-hp-bare"),
    ).resolves.toBeUndefined();
  });
});
