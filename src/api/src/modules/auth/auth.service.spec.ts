...
describe("AuthService", () => {
  ...
  describe("register", () => {
    ...
    it("should reject registration with duplicate email", async () => {
      ...
      await expect(
        service.register("taken@styx.protocol", "pass123", validRegisterOpts),
    ...
    it("should forward deviceFingerprint to antiSybil service if provided", async () => {
      ...
      expect(
        mockAntiSybil.registerDeviceFingerprint.mock.invocationCallOrder[0],
      ).toBeLessThan(
        (mockClient.query as jest.Mock).mock.invocationCallOrder.at(-1)!,
      );
    });

    it("rolls back signup when durable device registration fails", async () => {
      const mockAntiSybil = {
        registerDeviceFingerprint: jest
          .fn()
          .mockRejectedValue(new Error("device write failed")),
      } as any;
      const customService = new AuthService(mockPool, undefined, mockAntiSybil);
      (mockClient.query as jest.Mock)
      ...
      await expect(
        customService.register("fp-fail@styx.protocol", "pass123", {
          ...validRegisterOpts,
    ...
    it("should roll back account creation if user insert fails", async () => {
      ...
      await expect(
        service.register("race@styx.protocol", "pass123", validRegisterOpts),
      ).rejects.toThrow(
        "duplicate key value violates unique constraint users_email_key",
      );
  ...
  describe("login", () => {
    ...
    it("should reject login with wrong password", async () => {
      ...
      await expect(
        service.login("test@styx.protocol", "wrong-password"),
    ...
    it("should reject login with unknown email", async () => {
      ...
      await expect(
        service.login("unknown@styx.protocol", "any-password"),
      ).rejects.toThrow(UnauthorizedException);
    });
    ...
    it("should reject login when user has no password hash", async () => {
      ...
      await expect(
        service.login("test@styx.protocol", "any-password"),
      ).rejects.toThrow(UnauthorizedException);
    });
    ...
    it("should reject login when user account is inactive", async () => {
      ...
      await expect(
        service.login("test@styx.protocol", "correct-password"),
      ).rejects.toThrow(UnauthorizedException);
    });
  });
  ...
  describe("account lockout", () => {
    ...
    it("should lock account after 5 failed login attempts", async () => {
      ...
      await expect(
        service.login("lock@styx.protocol", "wrong-password"),
      ).rejects.toThrow(UnauthorizedException);
    ...
    it("should reject login when account is locked", async () => {
      ...
      await expect(
        service.login("lock@styx.protocol", "correct-password"),
  ...
  describe("JWT signing/verification", () => {
    it("should sign and verify tokens correctly", () => {
      ...
      const token = (service as any).signToken(
        "user-123",
        "test@styx.protocol",
      ); // allow-secret
  ...
  describe("enterprise SSO token exchange", () => {
    ...
    it("AU3: rejects enterprise SSO entirely when ENTERPRISE_SSO_SECRET is unset (no JWT_SECRET fallback)", async () => {
      ...
      await expect(service.exchangeEnterpriseToken(assertion)).rejects.toThrow(
    ...
    it("AU3: rejects a plain session token replayed as an SSO assertion when SSO is configured", async () => {
      ...
      const sessionToken = signAssertion(JWT_SECRET, {
        sub: "ent-user",
        email: "e@corp.com",
        role: "USER",
      }); // allow-secret
      ...
      await expect(
        service.exchangeEnterpriseToken(sessionToken),
      ).rejects.toThrow("Invalid enterprise token");
      expect(mockPool.query).not.toHaveBeenCalled();
    });

    it("rejects a token whose signature does not match the IdP secret (tampered/forged)", async () => {
      process.env.ENTERPRISE_SSO_SECRET = ENTERPRISE_SECRET;
      const assertion = signAssertion("wrong-secret", {
        sub: "ent-user",
        token_type: "enterprise_sso",
      }); // allow-secret

      await expect(service.exchangeEnterpriseToken(assertion)).rejects.toThrow(
        "Invalid enterprise token",
      );
      expect(mockPool.query).not.toHaveBeenCalled();
    });
    ...
    it("verifies against ENTERPRISE_SSO_SECRET when configured and rejects JWT_SECRET-signed tokens", async () => {
      ...
      const sessionStyle = signAssertion(JWT_SECRET, {
        sub: "ent-user",
        token_type: "enterprise_sso",
      });
      await expect(
        service.exchangeEnterpriseToken(sessionStyle),
      ).rejects.toThrow("Invalid enterprise token");
      expect(mockPool.query).not.toHaveBeenCalled();
    });
    ...
    it("accepts a token signed with ENTERPRISE_SSO_SECRET when configured", async () => {
      process.env.ENTERPRISE_SSO_SECRET = ENTERPRISE_SECRET;
      const assertion = signAssertion(ENTERPRISE_SECRET, {
        sub: "ent-user",
        email: "e@corp.com",
      });
    ...
    it("rejects when the enterprise user account is not active", async () => {
      ...
      await expect(service.exchangeEnterpriseToken(assertion)).rejects.toThrow(
        "Enterprise user account is not active",
      );
    });
  });
  ...
  describe("API keys", () => {
    it("should issue an API key and store only its hash", async () => {
      ...
      expect(insertCall[1][2]).toMatch(/^[a-f0-9]{128}$/);
        ...
        rows: [
          {
            id: "api-key-db-id",
            user_id: "user-123",
            key_hash: deriveApiKeyVerifier("correct-secret"),
            expires_at: new Date(Date.now() + 60_000).toISOString(),
            revoked_at: null,
            email: "test@styx.protocol",
            role: "USER",
    ...
    it("should revoke an API key by user and key id", async () => {
      ...
      await expect(
        service.revokeApiKey("user-123", "g".repeat(24)),
      ).resolves.toEqual({
        revoked: true,
      });
