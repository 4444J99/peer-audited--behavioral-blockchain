...
describe("ProofsController", () => {
  ...
  let mockR2: jest.Mocked<
    Pick<R2StorageService, "generateUploadUrl" | "downloadFile">
  >;
  ...
  let mockPhash: jest.Mocked<
    Pick<PHashService, "computeFrameHash" | "isDuplicate">
  >;
  let mockAnomaly: jest.Mocked<Pick<AnomalyService, "analyze">>;
  let mockProofsService: jest.Mocked<
    Pick<
      ProofsService,
      | "getProofUploadContractAccess"
      | "getProofUploadConfirmationAccess"
      | "getProofDetail"
    >
  >;
  let mockCrisisDetection: jest.Mocked<
    Pick<CrisisDetectionService, "analyzeContent">
  >;
  let mockCrisisIntervention: jest.Mocked<
    Pick<CrisisInterventionService, "reportCrisis">
  >;
  let mockVideoProcessing: jest.Mocked<
    Pick<VideoProcessingService, "dispatchForProcessing">
  >;
  ...
  describe("requestUploadUrl", () => {
    ...
    it("binds exactly one VALUES expression per column and one param per placeholder", async () => {
      ...
      expect(Math.max(...placeholders.map((p) => Number(p.slice(1))))).toBe(
        params.length,
      );
  ...
  describe("confirmUpload", () => {
    ...
    describe("capture provenance", () => {
      const primeConfirm = (issuedNonce: string | null) => {
        ...
        mockPhash.isDuplicate.mockResolvedValue({
          duplicate: false,
          closestDistance: 64,
        });
    ...
    it("should finalize proof with anomaly flags (no client biometric data persisted)", async () => {
      ...
      mockR2.downloadFile.mockResolvedValue(Buffer.from("fake-media"));
      mockAnomaly.analyze.mockResolvedValue({
        rejected: false,
        flags: ["EXIF_TIMESTAMP_DISCREPANCY"],
      });
      mockPhash.computeFrameHash.mockResolvedValue("hash-123");
      mockPhash.isDuplicate.mockResolvedValue({
        duplicate: false,
    ...
    it("enqueues redaction before routing the proof to reviewers", async () => {
      ...
      expect(mockVideoProcessing.dispatchForProcessing).toHaveBeenCalledWith(
        "p-1",
      );
      expect(
        mockVideoProcessing.dispatchForProcessing.mock.invocationCallOrder[0],
      ).toBeLessThan(
    ...
    it("still routes the proof when redaction dispatch fails (reviewers see no media, not raw media)", async () => {
      ...
      mockPool.query.mockResolvedValue({ rows: [] });
      mockVideoProcessing.dispatchForProcessing.mockRejectedValueOnce(
    ...
    it("should reject duplicate proofs", async () => {
      ...
      await expect(controller.confirmUpload("p-1", user, dto)).rejects.toThrow(
        ConflictException,
      );
    });
  });
  ...
  describe("processingComplete (SH15: per-proof scoping)", () => {
    ...
    it("fails closed (503) when the internal token is not configured", async () => {
      delete process.env.INTERNAL_SERVICE_TOKEN;
      await expect(
        controller.processingComplete("p-1", "anything", "challenge", {
          status: "COMPLETED",
        }),
    ...
    it("rejects a wrong internal token before touching the proof", async () => {
      await expect(
        controller.processingComplete("p-1", "wrong-token", "challenge", {
          status: "COMPLETED",
        }),
    ...
    it("rejects when the per-proof challenge token does not match (leaked global token alone is insufficient)", async () => {
      ...
      mockPool.query.mockResolvedValueOnce({
        rows: [
          {
            user_id: "u1",
            challenge_token: "proof-secret-1",
    ...
    it("finalizes and clears the challenge token when both tokens match and the proof is in-flight", async () => {
      ...
      const result = await controller.processingComplete(
        "p-1",
        "internal-token-abc",
        "proof-secret-1",
        {
