...
describe("ProofsService", () => {
  ...
  beforeEach(() => {
    ...
    mockR2 = {
      generateViewUrl: jest
        .fn()
        .mockResolvedValue("https://signed.example/view"),
    };
  ...
  it("allows the proof owner to fetch details with raw media and populated fields", async () => {
    ...
    const result = await service.getProofDetail("proof-1", {
      userId: "owner-1",
    });
  ...
  it("rejects unauthorized users", async () => {
    ...
    await expect(
      service.getProofDetail("proof-1", { userId: "intruder-1" }),
  ...
  it("throws when the proof does not exist", async () => {
    ...
    await expect(
      service.getProofDetail("missing-proof", { userId: "user-1" }),
