...
describe("UsersService (Full Breath Features)", () => {
  ...
  describe("setSelfExclusion", () => {
    it("should set self_exclusion_expires_at and log event", async () => {
      ...
      expect(mockPool.query).toHaveBeenCalledWith(
        expect.stringContaining(
          "UPDATE users SET self_exclusion_expires_at = $1",
        ),
    ...
    it("should throw BadRequestException for invalid duration", async () => {
      await expect(service.setSelfExclusion("u1", 0)).rejects.toThrow(
        BadRequestException,
      );
      await expect(service.setSelfExclusion("u1", 400)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
