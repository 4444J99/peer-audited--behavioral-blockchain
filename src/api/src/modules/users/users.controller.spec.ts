...
describe("UsersController", () => {
  ...
  describe("registerPushToken", () => {
    ...
    it("should throw for missing token", async () => {
      await expect(
        controller.registerPushToken({ id: "user-1" }, { token: "" }),
  ...
  describe("createCcpaDeletionRequest", () => {
    it("should create a deletion request for a California resident", async () => {
      ...
      const result = await controller.createCcpaDeletionRequest({
        id: "user-1",
      });

      expect(result).toEqual(request);
      expect(ccpaService.verifyCaliforniaResident).toHaveBeenCalledWith(
        "user-1",
      );
  ...
  describe("getCcpaDeletionRequestStatus", () => {
    it("should return the latest deletion request for the user", async () => {
      ...
      const result = await controller.getCcpaDeletionRequestStatus({
        id: "user-1",
      });

      expect(result).toEqual(request);
      expect(ccpaService.getDeletionRequestStatus).toHaveBeenCalledWith(
        "user-1",
      );
    });
