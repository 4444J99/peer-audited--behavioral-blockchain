...
describe("GdprService", () => {
  ...
  describe("exportUserData", () => {
    it("should export all user data in machine-readable format", async () => {
      ...
      (mockPool.query as jest.Mock)
        ...
        .mockResolvedValueOnce({
          rows: [
            { id: "c1", oath_category: "BIOLOGICAL_WEIGHT", status: "ACTIVE" },
          ],
        }) // contracts
        .mockResolvedValueOnce({
          rows: [{ id: "p1", contract_id: "c1", status: "PENDING" }],
        }) // proofs
        .mockResolvedValueOnce({ rows: [{ id: "e1", amount: "5000" }] }) // entries
        .mockResolvedValueOnce({
          rows: [{ id: "n1", type: "CONTRACT_UPDATE", title: "Test" }],
        }) // notifications
      ...
      expect(result.user).toEqual(
        expect.objectContaining({ id: userId, email: "test@styx.protocol" }),
      );
  ...
  describe("anonymization (via processPendingDeletions)", () => {
    ...
    it("should cancel a stored subscription before scrubbing Stripe linkage", async () => {
      ...
      const subscriptionLookupIndex = mockClient.query.mock.calls.findIndex(
        (call) =>
          ...
          call[0].includes("SELECT subscription_id"),
      );
    ...
    it("should scrub partner email, fury subject alias, and dashboard snapshots (PRV3)", async () => {
      ...
      expect(
        findClientCall("DELETE FROM dashboard_progress_snapshots"),
      ).toBeDefined();
    });
    ...
    it("should log GDPR_ERASURE_COMPLETED event in the same transaction", async () => {
      ...
      const payload = eventCall![1][2] as {
        userId: string;
        anonymizedAt: string;
      };
