...
const mockGenerateVCQuestions = generateVCQuestions as jest.MockedFunction<
  typeof generateVCQuestions
>;
const mockSimplifyConcept = simplifyConcept as jest.MockedFunction<
  typeof simplifyConcept
>;
...
describe("AiController", () => {
  ...
  describe("POST /ai/grill-me", () => {
    it("should return questions array for valid slideContent", async () => {
      ...
      const result = await controller.grillMe(
        "We are building a behavioral market...",
      );

      expect(result).toEqual({ questions: mockQuestions });
      expect(mockGenerateVCQuestions).toHaveBeenCalledWith(
        "We are building a behavioral market...",
      );
    });
    ...
    it("should return 503 when Gemini fails", async () => {
      mockGenerateVCQuestions.mockRejectedValue(
        new Error("Gemini API 500: Internal error"),
      );

      await expect(controller.grillMe("valid content")).rejects.toThrow(
        HttpException,
      );
      try {
  ...
  describe("POST /ai/eli5", () => {
    it("should return explanation for valid text", async () => {
      mockSimplifyConcept.mockResolvedValue(
        "Loss aversion means people hate losing more than they like winning.",
      );

      const result = await controller.eli5(
        "Explain loss aversion coefficient 1.955",
      );
      ...
      expect(mockSimplifyConcept).toHaveBeenCalledWith(
        "Explain loss aversion coefficient 1.955",
      );
    });
    ...
    it("should return 503 when Gemini fails", async () => {
      mockSimplifyConcept.mockRejectedValue(
        new Error("GEMINI_API_KEY not configured"),
      );

      await expect(controller.eli5("valid text")).rejects.toThrow(
        HttpException,
      );
      try {
