  ...
  describe("classify", () => {
    it("returns GENUINE_EMERGENCY when Gemini classifies as such", async () => {
      ...
      (callGemini as jest.Mock).mockResolvedValueOnce(
        JSON.stringify({
          category: "GENUINE_EMERGENCY",
          confidence: 0.95,
          reasoning: "Medical emergency described",
        }),
      );
    ...
    it("returns PURE_RATIONALIZATION with escalating response for repeat offenders", async () => {
      ...
      (callGemini as jest.Mock).mockResolvedValueOnce(
        JSON.stringify({
          ...
          confidence: 0.88,
          reasoning: "Classic avoidance pattern",
        }),
      );

      const result = await service.classify(
        "user-1",
        "I am too tired to do this today",
        "PROOF_FAILURE",
      );
    ...
    it("logs the classification to the database", async () => {
      ...
      (callGemini as jest.Mock).mockResolvedValueOnce(
        JSON.stringify({
          category: "PURE_RATIONALIZATION",
          confidence: 0.75,
          reasoning: "Victimhood narrative",
        }),
      );
      ...
      mockPool.query
        ...
        .mockResolvedValueOnce({
          rows: [
            {
              id: "r1",
              context_type: "GRACE_DAY",
              classification: "PURE_RATIONALIZATION",
              raw_text: "test",
              created_at: new Date(),
            },
          ],
