...
export class CorepayPayoutProvider implements PayoutProvider {
  ...
  async releaseFunds(
    ...
    if (!process.env.COREPAY_API_KEY) {
      this.logger.warn(
        "COREPAY_API_KEY not set — using dev fallback (no-op release)",
      );
      return {
    ...
    try {
      const response = await fetch(
        `${this.getBaseUrl()}/transactions/${paymentIntentId}/release`,
        {
          method: "POST",
  ...
  async captureFunds(
    ...
    if (!process.env.COREPAY_API_KEY) {
      this.logger.warn(
        "COREPAY_API_KEY not set — using dev fallback (no-op capture)",
      );
      return {
        status: PayoutStatus.SUCCESS,
    ...
    try {
      const response = await fetch(
        `${this.getBaseUrl()}/transactions/${paymentIntentId}/capture`,
        {
          method: "POST",
          headers: this.getHeaders(),
          body: JSON.stringify({
  ...
  private async handleResponse(
    ...
    if (!response.ok) {
      ...
      this.logger.error(
        `Corepay ${action} failed: HTTP ${response.status} — ${text}`,
      );
      return {
