...
describe("StripeFBOService", () => {
  ...
  beforeEach(() => {
    ...
    mockPaymentIntentsCapture = jest
      .fn()
      .mockResolvedValue({ id: "pi_captured", status: "succeeded" });
    mockPaymentIntentsCancel = jest
      .fn()
      .mockResolvedValue({ id: "pi_cancelled", status: "canceled" });
  ...
  describe("lockStakeInEscrow", () => {
    ...
    it("should return the PaymentIntent ID", async () => {
      ...
      const result = await service.lockStakeInEscrow(
        "user-2",
        10000,
        "contract-99",
      );
  ...
  describe("resolveEscrow (FAIL)", () => {
    it("should retrieve the intent and capture the whole stake on FAIL", async () => {
      ...
      const result = await service.resolveEscrow("pi_test_fail", "FAIL", [
        "fury-1",
      ]);
    ...
    it("should pay no Fury bounty, even with Furies present (DR-002)", async () => {
      ...
      mockPaymentIntentsRetrieve.mockResolvedValue({
        id: "pi_test_multi_fury",
        amount: 20000,
      });
      mockTransfersCreate.mockResolvedValue({ id: "tr_test_multi" });

      await service.resolveEscrow("pi_test_multi_fury", "FAIL", [
        "fury-A",
        "fury-B",
      ]);
    ...
    it("should also skip transfers when furies argument is omitted on FAIL", async () => {
      mockPaymentIntentsRetrieve.mockResolvedValue({
        id: "pi_test_default_furies",
        amount: 5000,
      });

      const result = await service.resolveEscrow(
        "pi_test_default_furies",
        "FAIL",
      );
    ...
    it("should reject a non-USD PaymentIntent rather than slashing it as USD (PM2)", async () => {
      ...
      await expect(
        service.resolveEscrow("pi_eur", "FAIL", ["fury-1"]),
  ...
  describe("fee split math", () => {
    it("should capture the entire forfeited stake to the platform (DR-002)", async () => {
      ...
      await service.resolveEscrow("pi_test_math", "FAIL", [
        "fury-1",
        "fury-2",
        "fury-3",
      ]);
