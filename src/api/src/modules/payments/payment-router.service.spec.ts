...
describe("PaymentRouterService", () => {
  ...
  beforeEach(() => {
    const mockStripe = {
      releaseFunds: jest
        .fn()
        .mockResolvedValue({ status: "COMPLETED", externalRef: "pi_mock_1" }),
    };
    const mockCorepay = {
      releaseFunds: jest
        .fn()
        .mockResolvedValue({ status: "COMPLETED", externalRef: "tok_mock_1" }),
    };
    service = new PaymentRouterService(mockStripe as any, mockCorepay as any);
  });
  ...
  describe("determineProcessor", () => {
    ...
    it("should route to HIGH_RISK_COREPAY when isHighRisk flag is set", () => {
      const result = service.determineProcessor(
        { ...baseOptions, isHighRisk: true },
        0,
      );
      expect(result).toBe("HIGH_RISK_COREPAY");
    });
  ...
  describe("createPaymentIntent", () => {
    ...
    it("should throw in production when STRIPE_SECRET_KEY is missing", async () => {
      ...
      await expect(
        service.createPaymentIntent(baseOptions, "STRIPE"),
      ).rejects.toThrow();
    });
    ...
    it("should throw for COREPAY in production", async () => {
      process.env.NODE_ENV = "production";
      await expect(
        service.createPaymentIntent(baseOptions, "HIGH_RISK_COREPAY"),
      ).rejects.toThrow();
    });
  });
  ...
  describe("determineGeographicRoute", () => {
    ...
    it("blocks payments from hard-blocked US state (WA)", () => {
      expect(() => {
        service.determineGeographicRoute(
          baseOptions,
          { country: "US", state: "WA" },
          0,
        );
