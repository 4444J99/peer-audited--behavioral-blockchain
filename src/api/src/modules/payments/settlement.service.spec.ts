...
describe("SettlementService", () => {
  ...
  let mockCompliancePolicy: {
    evaluateKycRequirement: jest.Mock;
  ...
  describe("dispatchSettlement", () => {
    it("should add a job to the queue", async () => {
      ...
      expect(
        mockCompliancePolicy.evaluateKycRequirement,
      ).not.toHaveBeenCalled();
    });

    it("should reject settlement jobs without a rail hold id", async () => {
      const job: SettlementJob = {
        contractId: "c-no-hold",
        outcome: "PASS",
        amountCents: 1000,
      };

      await expect(service.dispatchSettlement(job)).rejects.toThrow(
        /escrow hold id/,
      );
    ...
    it("should enforce KYC for amounts > $20.00 when compliance policy denies", async () => {
      ...
      mockCompliancePolicy.evaluateKycRequirement.mockResolvedValue({
        allowed: false,
        reason:
          "Identity verification required for stakes above $20. Complete KYC to continue.",
      });
      ...
      expect(mockCompliancePolicy.evaluateKycRequirement).toHaveBeenCalledWith(
        "user-1",
        25,
      );
    });
    ...
    it("should allow large settlements if compliance policy allows them", async () => {
      ...
      mockCompliancePolicy.evaluateKycRequirement.mockResolvedValueOnce({
        allowed: true,
      });
  ...
  describe("getSettlementPreview", () => {
    ...
    it("should throw NotFoundException if contract missing", async () => {
      mockPool.query.mockResolvedValueOnce({ rows: [] });
      await expect(service.getSettlementPreview("missing")).rejects.toThrow(
        NotFoundException,
      );
    });
