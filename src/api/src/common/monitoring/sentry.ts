...
export function initSentry(): void {
  ...
  try {
    ...
    console.warn(
      "[Sentry] @sentry/nestjs not installed — run: npm install @sentry/nestjs",
    );
  }
}
...
export function captureFinancialAlert(
  ...
  try {
    if (typeof SentryModule.withScope === "function") {
      SentryModule.withScope((scope: any) => {
        ...
        SentryModule.captureMessage(
          `FINANCIAL INTEGRITY ALERT: ${event}`,
          "error",
        );
      });
    } else {
    ...
    console.error(
      `Failed to dispatch Sentry financial alert: ${(err as Error).message}`,
      details,
    );
  }
}
