...
describe("Migration Runner", () => {
  ...
  describe("getPendingMigrations", () => {
    ...
    it("should throw Error on schema drift when applied migration is missing from files", async () => {
      ...
      await expect(getPendingMigrations(mockPool)).rejects.toThrow(
        "Schema drift detected: Applied migration 001_initial_schema.sql is missing from the filesystem.",
      );
    });
    ...
    it("should throw Error when applied migrations are not a contiguous prefix", async () => {
      ...
      await expect(getPendingMigrations(mockPool)).rejects.toThrow(
        "Schema drift detected: Pending migration 001_initial_schema.sql precedes applied migration 002_add_index.sql.",
      );
    });
  ...
  describe("allTablesExist", () => {
    ...
    it("returns true when information_schema reports every table", async () => {
      ...
      await expect(
        allTablesExist(mockPool, ["accounts", "users"]),
      ).resolves.toBe(true);
    ...
    it("returns false when any table is missing", async () => {
      ...
      await expect(
        allTablesExist(mockPool, ["accounts", "pod_broadcast_log"]),
      ).resolves.toBe(false);
    });
  });
  ...
  describe("baselineFromExistingSchema", () => {
    it("stamps only the contiguous prefix whose tables exist and stops at the first gap", async () => {
      ...
      routeMigrationSql({
        "001_initial_schema.sql":
          "CREATE TABLE accounts (id UUID); CREATE TABLE users (id UUID);",
        "009_missing_base_tables.sql":
          "CREATE TABLE IF NOT EXISTS attestations (id UUID);",
        "050_behavioral_omega_tables.sql":
