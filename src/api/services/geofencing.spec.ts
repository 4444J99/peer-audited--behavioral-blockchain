...
describe("geofencing", () => {
  ...
  describe("STATE_TIERS", () => {
    ...
    it("should never capture a forfeited stake in a survey-blocked state", () => {
      for (const state of ["NV", "SD", "AZ", "MT"]) {
        expect(
          classifyJurisdiction({ country: "US", region: state }).tier,
        ).not.toBe(JurisdictionTier.TIER_1);
      }
    });
  ...
  describe("classifyJurisdiction", () => {
    ...
    it("should resolve a full state name to its code and tier (SH8)", () => {
      expect(
        classifyJurisdiction({ country: "US", region: "California" }),
      ).toEqual({
        tier: JurisdictionTier.TIER_1,
        state: "CA",
      });
      expect(
        classifyJurisdiction({ country: "US", region: "new york" }),
      ).toEqual({
