...
export class FitbitSyncService {
  ...
  private assertConfigured(): void {
    if (!this.isConfigured()) {
      throw new ServiceUnavailableException(
        "Fitbit integration is not configured",
      );
    }
  }
    ...
    try {
      ...
      if (err?.code === "23505") {
        throw new ConflictException(
          "This Fitbit account is already linked to another user",
        );
      }
      throw err;
    }

    const subscribed = await this.ensureSleepSubscription(
      token.access_token,
      userId,
    );
    return {
  ...
  async disconnectUser(userId: string): Promise<{ disconnected: boolean }> {
    ...
    await this.pool.query(
      `DELETE FROM fitbit_oauth_tokens WHERE user_id = $1`,
      [userId],
    );
  ...
  async getValidAccessToken(
    userId: string,
  ): Promise<{ accessToken: string; fitbitUserId: string }> {
    this.assertConfigured();
    ...
    if (result.rows.length === 0) {
      throw new NotFoundException(
        "Fitbit account is not connected for this user",
      );
    }
    ...
    if (
      row.access_token &&
      expiresAtMs - ACCESS_TOKEN_EXPIRY_MARGIN_MS > Date.now()
    ) {
      return {
  ...
  async fetchDailySummary(
    ...
    const main =
      ...
      (deviceLogs.length > 0
        ? deviceLogs.reduce((longest, log) =>
            Number(log?.minutesAsleep || 0) >
            Number(longest?.minutesAsleep || 0)
              ? log
              : longest,
          )
        : undefined);
    ...
    const summary: FitbitDailySummary = {
      ...
      deviceLoggedMainSleep: main
        ? {
            ...
            efficiency:
              main.efficiency !== undefined
                ? Number(main.efficiency)
                : undefined,
            deepSleepMinutes:
    ...
    try {
      const heartBody = await this.fitbitGet(
        `/1/user/-/activities/heart/date/${date}/1d.json`,
        accessToken,
      );
      const resting =
        heartBody?.["activities-heart"]?.[0]?.value?.restingHeartRate;
  ...
  private async ensureSleepSubscription(
    accessToken: string,
    userId: string,
  ): Promise<boolean> {
    try {
      ...
      this.logger.warn(
        `Fitbit sleep subscription for user ${userId} failed (${res.status})`,
      );
      return false;
    } catch (err) {
      this.logger.warn(
  ...
  private async requestToken(
    ...
    if (!res.ok) {
      ...
      throw new BadRequestException(
        `Fitbit ${grantType} token request failed (${res.status})`,
      );
    }
    ...
    if (!body.access_token || !body.refresh_token || !body.user_id) {
      throw new BadRequestException(
        "Fitbit token response is missing required fields",
      );
    }
    return body;
  }
  ...
  private async fitbitGet(path: string, accessToken: string): Promise<any> {
    ...
    if (!res.ok) {
      throw new ServiceUnavailableException(
        `Fitbit API request ${path} failed (${res.status})`,
      );
    }
    return res.json();
  }
