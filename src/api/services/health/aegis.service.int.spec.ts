...
describe("AegisProtocolService (Integration)", () => {
  ...
  describe("penalty multiplier windows", () => {
    it("applies a 1.5x volatility multiplier on Friday and Saturday nights", () => {
      expect(service.getVolatilityMultiplier(new Date(2026, 5, 5, 21))).toBe(
        1.5,
      );
      expect(service.getVolatilityMultiplier(new Date(2026, 5, 6, 3))).toBe(
        1.5,
      );
    });

    it("does not apply the volatility multiplier outside the weekend-night window", () => {
      expect(service.getVolatilityMultiplier(new Date(2026, 5, 5, 20))).toBe(
        1.0,
      );
      expect(service.getVolatilityMultiplier(new Date(2026, 5, 6, 4))).toBe(
        1.0,
      );
      expect(service.getVolatilityMultiplier(new Date(2026, 5, 7, 22))).toBe(
        1.0,
      );
    });
  });
