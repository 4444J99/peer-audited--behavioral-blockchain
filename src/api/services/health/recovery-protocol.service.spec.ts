...
import {
  ...
  RecoveryMetadata,
} from "./recovery-protocol.service";
import {
...
describe("RecoveryProtocolService", () => {
  ...
  it("should accept a valid RECOVERY_NOCONTACT contract", async () => {
    const result = await service.validateRecoveryContract(
      "user-1",
      "RECOVERY_NOCONTACT",
      14,
      validMetadata,
  ...
  it("should reject when metadata is missing", async () => {
    await expect(
      service.validateRecoveryContract(
        "user-1",
        "RECOVERY_NOCONTACT",
        14,
        undefined,
      ...
      mockPool.query
        ...
        .mockResolvedValueOnce({
          rows: [
            {
              metadata: {
                noContactIdentifiers: ["e1", "e2", "e3", "e4", "e5", "e6"],
              },
              oath_category: "RECOVERY_NOCONTACT",
            },
            {
