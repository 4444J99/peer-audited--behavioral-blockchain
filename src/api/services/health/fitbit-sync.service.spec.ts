...
describe("FitbitSyncService", () => {
  ...
  describe("connectUser", () => {
    it("exchanges the code with Basic auth, persists the grant, and subscribes", async () => {
      ...
      const result = await service.connectUser(
        "user-1",
        "auth-code",
      ...
      expect(subUrl).toBe(
        "https://api.fitbit.com/1/user/-/sleep/apiSubscriptions/user-1.json",
      );
    ...
    it("maps a fitbit_user_id unique violation to ConflictException (anti-Sybil)", async () => {
      ...
      await expect(service.connectUser("user-2", "auth-code")).rejects.toThrow(
        ConflictException,
      );
    });
    ...
    it("rejects a failed token exchange", async () => {
      fetchMock.mockResolvedValueOnce(
        jsonResponse(400, { errors: [{ errorType: "invalid_grant" }] }),
      );

      await expect(service.connectUser("user-1", "bad-code")).rejects.toThrow(
        BadRequestException,
      );
  ...
  describe("getValidAccessToken", () => {
    ...
    it("throws UnauthorizedException when the refresh grant is rejected", async () => {
      pool.query.mockResolvedValueOnce({
        rows: [
          freshTokenRow({ access_token: null, access_token_expires_at: null }),
        ],
        rowCount: 1,
      } as any);
      fetchMock.mockResolvedValueOnce(
        jsonResponse(401, { errors: [{ errorType: "invalid_grant" }] }),
      );
  ...
  describe("fetchDailySummary", () => {
    ...
    it("parses device-detected main sleep and resting heart rate", async () => {
      ...
      fetchMock
            ...
            sleep: [
              {
                ...
                efficiency: 93,
                levels: {
                  summary: { deep: { minutes: 80 }, rem: { minutes: 100 } },
                },
              },
              {
                logId: 1,
        ...
        .mockResolvedValueOnce(
          jsonResponse(200, {
            "activities-heart": [
              { dateTime: "2026-07-29", value: { restingHeartRate: 52 } },
            ],
          }),
        );
      ...
      expect(sleepUrl).toBe(
        "https://api.fitbit.com/1.2/user/-/sleep/date/2026-07-29.json",
      );
    ...
    it("surfaces a failed sleep fetch as ServiceUnavailableException", async () => {
      ...
      await expect(
        service.fetchDailySummary("user-1", "2026-07-29"),
  ...
  describe("resolveUserByFitbitId", () => {
    it("returns the mapped user id", async () => {
      ...
      await expect(service.resolveUserByFitbitId("FB123")).resolves.toBe(
        "user-1",
      );
    });
  ...
  describe("disconnectUser", () => {
    it("returns disconnected: false when no grant exists", async () => {
      ...
      await expect(service.disconnectUser("user-1")).resolves.toEqual({
        disconnected: false,
      });
      expect(fetchMock).not.toHaveBeenCalled();
    });
    ...
    it("removes the grant even when subscription teardown fails", async () => {
      ...
      expect(String(deleteCall[0])).toContain(
        "DELETE FROM fitbit_oauth_tokens",
      );
