...
describe("TruthLogService", () => {
  ...
  it("should append event using GENESIS_HASH if table is empty", async () => {
    ...
    const expectedHash = createHash("sha256")
      .update(
        `1|TEST_EVENT|${fixedDate.toISOString()}|${GENESIS_HASH}|${JSON.stringify(payload)}`,
      )
      .digest("hex");
  ...
  it("should append event chaining the previous hash correctly", async () => {
    ...
    mockClient.query
      ...
      .mockResolvedValueOnce({
        rows: [{ sequence_index: "10", current_hash: "abc123oldhash" }],
      }) // SELECT latest
    ...
    const expectedHash = createHash("sha256")
      .update(
        `11|TEST_EVENT|${fixedDate.toISOString()}|abc123oldhash|${JSON.stringify(payload)}`,
      )
      .digest("hex");
  ...
  describe("verifyChain", () => {
    it("should return valid if chain is consistent", async () => {
      ...
      const hash1 = createHash("sha256")
        .update(
          `1|E1|${fixedDate.toISOString()}|${GENESIS_HASH}|${JSON.stringify(payload1)}`,
        )
        .digest("hex");
      ...
      const hash2 = createHash("sha256")
        .update(
          `2|E2|${fixedDate.toISOString()}|${hash1}|${JSON.stringify(payload2)}`,
        )
        .digest("hex");
    ...
    it("LC3: should detect a consistently re-forged chain (links agree internally but diverge from genesis)", async () => {
      ...
      const forgedHash1 = createHash("sha256")
        .update(
          `1|E1|${fixedDate.toISOString()}|${forgedPrev1}|${JSON.stringify(forgedPayload1)}`,
        )
        .digest("hex");
      const payload2 = { b: 2 };
      const forgedHash2 = createHash("sha256")
        .update(
          `2|E2|${fixedDate.toISOString()}|${forgedHash1}|${JSON.stringify(payload2)}`,
        )
        .digest("hex");
        ...
        rows: [
          {
            ...
            sequence_index: 1,
            event_type: "E1",
            payload: forgedPayload1,
            previous_hash: forgedPrev1,
            current_hash: forgedHash1,
            created_at: fixedDate,
          },
          {
            id: "2",
            ...
            event_type: "E2",
            payload: payload2,
            previous_hash: forgedHash1,
            current_hash: forgedHash2,
            created_at: fixedDate,
          },
        ],
      });
