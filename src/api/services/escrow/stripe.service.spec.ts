...
describe("StripeFboService (dev mode)", () => {
  ...
  describe("captureStake", () => {
    ...
    describe("idempotent retry (non-dev mode)", () => {
      ...
      let mockStripe: {
        paymentIntents: { retrieve: jest.Mock; capture: jest.Mock };
      };
      ...
      beforeEach(() => {
        ...
        Object.defineProperty(liveService, "movesRealMoney", {
          get: () => true,
        });
  ...
  describe("transferFunds (PM7 idempotency)", () => {
    ...
    beforeEach(() => {
      ...
      mockStripe = {
        transfers: {
          create: jest
            .fn()
            .mockResolvedValue({ id: "tr_live_1", amount: 1000 }),
        },
      };
      Object.defineProperty(liveService, "isMockMode", { get: () => false });
    ...
    it("should attach a deterministic idempotency key derived from a stable metadata id", async () => {
      await liveService.transferFunds(1000, "acct_dest_1", {
        sideEffectKey: "evt-123",
      });

      expect(mockStripe.transfers.create).toHaveBeenCalledWith(
        expect.objectContaining({
          amount: 1000,
          destination: "acct_dest_1",
          currency: "usd",
        }),
    ...
    it("should honor an explicit idempotency key argument", async () => {
      await liveService.transferFunds(
        500,
        "acct_dest_2",
        undefined,
        "explicit-key",
  ...
  describe("resolveDisposition", () => {
    it("should return REFUND for completed contracts in all tiers", () => {
      expect(
        service.resolveDisposition("COMPLETED", JurisdictionTier.TIER_1),
      ).toBe("REFUND");
      expect(
        service.resolveDisposition("COMPLETED", JurisdictionTier.TIER_2),
      ).toBe("REFUND");
      expect(
        service.resolveDisposition("COMPLETED", JurisdictionTier.TIER_3),
      ).toBe("REFUND");
    });

    it("should return CAPTURE for failed TIER_1 contracts (platform revenue)", () => {
      expect(
        service.resolveDisposition("FAILED", JurisdictionTier.TIER_1),
      ).toBe("CAPTURE");
    });

    it("should return REFUND for failed TIER_2 contracts (refund-only jurisdiction)", () => {
      expect(
        service.resolveDisposition("FAILED", JurisdictionTier.TIER_2),
      ).toBe("REFUND");
    });

    it("should return REFUND for failed TIER_3 contracts (safety fallback)", () => {
      expect(
        service.resolveDisposition("FAILED", JurisdictionTier.TIER_3),
      ).toBe("REFUND");
    });
  });
});
...
describe("StripeFboService real-money interlocks", () => {
  ...
  it("refuses to hold real money while the geofence fails open", async () => {
    ...
    await expect(
      service.holdStake("cus_1", 3000, "contract-1"),
    ).rejects.toThrow(/geofence fails open/i);
  });
  ...
  it("refuses to hold real money while KYC enforcement is off", async () => {
    ...
    await expect(
      service.holdStake("cus_1", 3000, "contract-1"),
    ).rejects.toThrow(/KYC enforcement is disabled/i);
  });
  ...
  it("refuses to hold real money while test-money mode is on", async () => {
    ...
    await expect(
      service.holdStake("cus_1", 3000, "contract-1"),
    ).rejects.toThrow(/test-money pilot/i);
  });
  ...
  it("guards capture and transfer, not just the initial hold", async () => {
    ...
    await expect(service.captureStake("pi_1")).rejects.toThrow(
      /test-money pilot/i,
    );
    await expect(service.transferFunds(1000, "acct_1")).rejects.toThrow(
      /test-money pilot/i,
    );
  });
