...
export class SelfExclusionService {
  ...
  async activate(
    ...
    if (durationDays < 1)
      throw new BadRequestException("Minimum exclusion is 1 day");
    if (durationDays > 365)
      throw new BadRequestException("Maximum self-exclusion is 1 year");
    ...
    this.logger.log(
      `User ${userId} self-excluded until ${expiresAt.toISOString()}`,
    );
    return { expiresAt };
  }
  ...
  async isExcluded(
    userId: string,
  ): Promise<{ excluded: boolean; expiresAt?: Date }> {
    ...
    return {
      excluded: isExcluded,
      expiresAt: isExcluded ? new Date(expiresAt) : undefined,
    };
  }
}
