...
describe("GeofenceService", () => {
  ...
  describe("checkJurisdiction", () => {
    ...
    it("should throw ForbiddenException for restricted TIER_3 state (WA)", () => {
      ...
      expect(() => service.checkJurisdiction("1.2.3.4")).toThrow(
        ForbiddenException,
      );
      expect(() => service.checkJurisdiction("1.2.3.4")).toThrow(
        /Jurisdiction Violation.*WA/,
      );
    });
    ...
    it("should throw ForbiddenException for a non-US IP (fail-closed)", () => {
      ...
      expect(() => service.checkJurisdiction("1.2.3.4")).toThrow(
        /Jurisdiction Violation.*Non-US\/Unknown/,
      );
    });

    it("should throw ForbiddenException for an unresolvable IP (fail-closed)", () => {
      mockLookup.mockReturnValue(null);
      expect(() => service.checkJurisdiction("0.0.0.0")).toThrow(
        ForbiddenException,
      );
      expect(() => service.checkJurisdiction("0.0.0.0")).toThrow(
        /Jurisdiction Violation.*Non-US\/Unknown/,
