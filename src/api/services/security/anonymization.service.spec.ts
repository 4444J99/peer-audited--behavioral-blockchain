...
describe("AnonymizationService", () => {
  ...
  describe("hash (keyed HMAC, PRV8)", () => {
    ...
    it("should be case-insensitive", () => {
      expect((service as any).hash("Test@Example.COM")).toBe(
        (service as any).hash("test@example.com"),
      );
    });

    it("should return a 64-char hex string (HMAC-SHA256)", () => {
      expect((service as any).hash("test@example.com")).toMatch(
        /^[0-9a-f]{64}$/,
      );
    });

    it("should produce different hashes for different emails", () => {
      expect((service as any).hash("alice@example.com")).not.toBe(
        (service as any).hash("bob@example.com"),
      );
    });
