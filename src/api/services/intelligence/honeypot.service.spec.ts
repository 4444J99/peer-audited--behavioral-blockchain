    ...
    it("should query for furies and inject honeypot proof", async () => {
      (mockPool.query as jest.Mock)
        ...
        .mockResolvedValueOnce({
          rows: [{ id: "contract-abc", user_id: "user-xyz" }],
        }) // host contract
        .mockResolvedValueOnce({ rows: [{ id: "proof-hp-123" }] }); // proof insert

      (mockRouter.routeProof as jest.Mock).mockResolvedValueOnce(
        "mock-job-123",
      );

      await honeypotService.injectHoneypot();

      expect(mockRouter.routeProof).toHaveBeenCalledWith(
        "proof-hp-123",
        "user-xyz",
        3,
      );
      ...
      const [insertSql, insertParams] = (
        ...
        ([sql]: [string]) =>
          typeof sql === "string" && sql.includes("INSERT INTO proofs"),
      ) as [string, unknown[]];
  ...
  describe("gradeHoneypotPerformance", () => {
    ...
    it("should penalize furies who incorrectly voted PASS on honeypot and SHADOW_BAN if below threshold", async () => {
      gradeClient.query
        .mockResolvedValueOnce({ rows: [] }) // BEGIN
        .mockResolvedValueOnce({
          rows: [{ fury_user_id: "fury-3", verdict: "PASS" }],
        }) // SELECT assignments
      ...
      const statusUpdateCalls = gradeClient.query.mock.calls.filter(
        ([sql]: [string]) =>
          typeof sql === "string" && sql.includes("status = 'SHADOW_BANNED'"),
      );
    ...
    it("should ROLLBACK and re-throw on database error", async () => {
      ...
      await expect(
        honeypotService.gradeHoneypotPerformance("proof-hp-4", []),
