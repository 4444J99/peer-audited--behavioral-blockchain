...
describe("Ask Styx Worker", () => {
  ...
  it("sets CORS headers using ALLOWED_ORIGIN env", async () => {
    ...
    const res = await worker.fetch(
      req,
      makeEnv({ ALLOWED_ORIGIN: "https://custom.io" }) as any,
    );
  ...
  describe("CORS origin resolution", () => {
    it("matches requested origin when present in comma-separated allowed list", () => {
      const allowed =
        "https://organvm-iii-ergon.github.io, https://4444j99.github.io, http://localhost:5173";
      expect(resolveAllowedOrigin("https://4444j99.github.io", allowed)).toBe(
        "https://4444j99.github.io",
      );
      expect(
        resolveAllowedOrigin("https://organvm-iii-ergon.github.io", allowed),
      ).toBe("https://organvm-iii-ergon.github.io");
      expect(resolveAllowedOrigin("http://localhost:5173", allowed)).toBe(
        "http://localhost:5173",
      );
    });
    ...
    it("falls back to first allowed origin when request origin does not match", () => {
      const allowed =
        "https://organvm-iii-ergon.github.io, https://4444j99.github.io";
      expect(resolveAllowedOrigin("https://malicious.com", allowed)).toBe(
        "https://organvm-iii-ergon.github.io",
      );
    });

    it("falls back to request origin or wildcard when no allowed origin is configured", () => {
      expect(resolveAllowedOrigin("https://example.com", undefined)).toBe(
        "https://example.com",
      );
    ...
    it("reflects matching origin header in response", async () => {
      ...
      const res = await worker.fetch(
        req,
        makeEnv({ ALLOWED_ORIGIN: allowed }) as any,
      );
      expect(res.headers.get("Access-Control-Allow-Origin")).toBe(
        "https://4444j99.github.io",
      );
