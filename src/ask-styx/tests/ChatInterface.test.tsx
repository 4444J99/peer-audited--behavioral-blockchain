...
describe("ChatInterface", () => {
  ...
  it("shows suggested questions when no messages", () => {
    render(<ChatInterface />);
    expect(
      screen.getByText("How does the integrity score work?"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("What are the 7 oath categories?"),
    ).toBeInTheDocument();
  });

  it("shows description text when no messages", () => {
    render(<ChatInterface />);
    expect(screen.getByText(/Ask anything about Styx/)).toBeInTheDocument();
  });

  it("renders the input textarea", () => {
    render(<ChatInterface />);
    expect(
      screen.getByPlaceholderText("Ask about Styx..."),
    ).toBeInTheDocument();
  });
  ...
  it("does not show suggested questions when messages exist", () => {
    const stored = [{ role: "user", content: "test", timestamp: Date.now() }];
    localStorageMock.getItem.mockReturnValueOnce(JSON.stringify(stored));
    render(<ChatInterface />);
    expect(
      screen.queryByText("How does the integrity score work?"),
