...
jest.mock("expo-crypto", () => ({
  ...
  getRandomBytesAsync: jest.fn(async (len: number) =>
    Uint8Array.from({ length: len }, (_, i) => i + 1),
  ),
  digestStringAsync: jest.fn(
    async (_alg: string, payload: string) => `hash:${payload}`,
  ),
}));
...
describe("ZKPrivacyEngine", () => {
  ...
  it("returns null from compatibility breach API when no matching local event exists", async () => {
    ZKPrivacyEngine.setLogProvider(async () => [
      {
        counterparty: "+15550001111",
        timestamp: "2026-03-03T12:00:00.000Z",
        channel: "SMS",
      },
    ]);

    const proof = await ZKPrivacyEngine.generateBreachProof(
      "+1 (555) 111-2222",
      {
        start: new Date("2026-03-03T00:00:00.000Z"),
        end: new Date("2026-03-04T00:00:00.000Z"),
  ...
  it("generates breach proof from normalized identifier using latest event", async () => {
    ZKPrivacyEngine.setLogProvider(async () => [
      {
        counterparty: "+1 (555) 111-2222",
        timestamp: "2026-03-03T01:00:00.000Z",
        channel: "SMS",
      },
      {
        identifier: "15551112222",
        timestamp: new Date("2026-03-03T22:00:00.000Z"),
        method: "CALL",
      },
    ]);
