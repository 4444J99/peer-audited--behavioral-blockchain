...
function jsonFailWithRequestId(
  status: number,
  body: unknown,
  requestId: string,
) {
...
describe("ApiClient", () => {
  ...
  describe("request()", () => {
    it("sends Content-Type and Authorization headers", async () => {
      ...
      mockFetch.mockResolvedValueOnce(
        jsonOk({ userId: "1", token: "x", integrity: 50 }),
      );
    ...
    it("throws on non-ok response with status and body", async () => {
      ...
      await expect(ApiClient.login("a@b.com", "pw")).rejects.toThrow(
        "API 403: Forbidden",
      );
    });
  });
  ...
  describe("login()", () => {
    it("sends POST to /auth/login with email and password", async () => {
      mockFetch.mockResolvedValueOnce(
        jsonOk({ userId: "u1", token: "t1", integrity: 50 }),
      );
  ...
  describe("submitProof()", () => {
    it("sends POST to /contracts/:id/proof with mediaUri", async () => {
      ...
      const result = await ApiClient.submitProof("contract-42", {
        mediaUri: "local://proof/camera/abc",
      });
