...
describe("UploadService", () => {
  ...
  it("requestPreSignedUrl() throws on non-OK response", async () => {
    ...
    await expect(
      UploadService.requestPreSignedUrl("contract_123", "video/mp4"),
  ...
  it("confirmUpload() calls confirmation endpoint and returns true", async () => {
    ...
    const ok = await UploadService.confirmUpload(
      "proof_123",
...
describe("UploadService.pollProcessingStatus()", () => {
      ...
      status("FAILED", [
        {
          stage: "VALIDATE",
          status: "FAILED",
          error: "Source video is not decodable",
          updated_at: "t1",
        },
      ]),
    );
  ...
  it("backs off exponentially from 2s and caps at 30s", async () => {
    const clock = createVirtualClock();
    (ApiClient.getProcessingStatus as jest.Mock).mockResolvedValue(
      status("PROCESSING"),
    );
    ...
    expect(clock.slept.slice(0, 5)).toEqual([
  ...
  it("surfaces the last request error when the deadline passes without a reading", async () => {
    const clock = createVirtualClock();
    (ApiClient.getProcessingStatus as jest.Mock).mockRejectedValue(
      new Error("API 503"),
    );
  ...
  it("resolves null and stops polling once the caller cancels", async () => {
    const clock = createVirtualClock();
    let cancelled = false;
    (ApiClient.getProcessingStatus as jest.Mock).mockImplementation(
      async () => {
        cancelled = true;
