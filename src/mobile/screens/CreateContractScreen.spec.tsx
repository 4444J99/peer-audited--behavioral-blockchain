...
describe("CreateContractScreen – render", () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    (ApiClient.createContract as jest.Mock).mockResolvedValue({
  ...
  function renderScreen() {
    return render(
      React.createElement(CreateContractScreen, {
        route: mockRoute,
        navigation: mockNav,
      }),
    );
  ...
  it("blocks submit when stake is below minimum bound", async () => {
    ...
    fireEvent.click(
      getByText("No Contact Boundary").closest("button") as HTMLElement,
    );
    fireEvent.click(
      getByText("Screen Time API").closest("button") as HTMLElement,
    );
    fireEvent.change(
    ...
    fireEvent.click(
      getByText("I am entering this contract voluntarily.").closest(
        "button",
      ) as HTMLElement,
    );
    fireEvent.click(
      getByText("No minors are involved in this contract.").closest(
        "button",
      ) as HTMLElement,
    );
    fireEvent.click(
      getByText("No dependents are affected by this commitment.").closest(
        "button",
      ) as HTMLElement,
    );
    fireEvent.click(
      getByText("This does not violate any legal obligations.").closest(
        "button",
      ) as HTMLElement,
    );
    fireEvent.change(getByPlaceholderText("0.00"), { target: { value: "5" } });
    fireEvent.click(
      getByText("STAKE AND COMMIT").closest("button") as HTMLElement,
    );

    await waitFor(() => {
      expect(container.textContent).toContain(
        "Stake amount must be between $10 and $200.",
      );
  ...
  it("submits valid bounded stake and navigates to contract detail", async () => {
    ...
    fireEvent.click(
      getByText("Fury Peer Review").closest("button") as HTMLElement,
    );
    fireEvent.change(
      getByPlaceholderText("Describe your behavioral commitment..."),
      {
