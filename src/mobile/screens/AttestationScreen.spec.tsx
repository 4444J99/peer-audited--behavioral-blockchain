...
describe("AttestationScreen – trace-ID display", () => {
  ...
  it("renders trace ID for non-recovery contract error", () => {
    const tree = SupportTraceErrorBanner({
      value:
        "Attestation only available for recovery contracts [request_id: att-cat-err]",
    }) as any;
    const text = collectText(tree);
...
describe("AttestationScreen – render", () => {
  ...
  function renderScreen() {
    return render(
      React.createElement(AttestationScreen, {
        route: mockRoute,
        navigation: mockNav,
      }),
    );
  ...
  it("calls getAttestationStatus with the correct contractId", () => {
    renderScreen();
    expect(ApiClient.getAttestationStatus).toHaveBeenCalledWith(
      "test-contract-123",
    );
  });
  ...
  it("renders snake_case attestation status data from the API", async () => {
    ...
    expect(
      getByText("1 missed attestation — 2 remaining before auto-fail"),
    ).toBeTruthy();
  ...
  it("submits a daily attestation and renders the confirmed state", async () => {
    ...
    fireEvent.click(
      getByText("I HELD THE LINE").closest("button") as HTMLElement,
    );

    await waitFor(() => {
      expect(ApiClient.submitAttestation).toHaveBeenCalledWith(
        "test-contract-123",
      );
