...
import { MAX_WEEKLY_LOSS_VELOCITY_PCT } from "@styx/types";
...
const OATH_CATEGORIES = [
  ...
  {
    value: "RECOVERY_SUBSTANCE",
    label: "Substance Abstinence",
    stream: "Recovery",
  },
  { value: "RECOVERY_DETOX", label: "Behavioral Detox", stream: "Recovery" },
  {
    value: "BIOLOGICAL_WEIGHT",
    label: "Weight Management",
    stream: "Biological",
  },
  {
    value: "BIOLOGICAL_CARDIO",
...
export function CreateContractScreen({ navigation }: Props) {
  ...
  const boundedStake = Math.min(
    MAX_STAKE_USD,
    Math.max(MIN_STAKE_USD, normalizedStake),
  );
  ...
  const handleSubmit = async () => {
    ...
    if (amount < MIN_STAKE_USD || amount > MAX_STAKE_USD) {
      setError(
        `Stake amount must be between $${MIN_STAKE_USD} and $${MAX_STAKE_USD}.`,
      );
      return;
    }
      ...
      const result = await ApiClient.createContract({
        ...
        recoveryMetadata: isRecovery
          ? {
              ...
              noContactIdentifiers: targetIdentifiers.filter(
                (id) => id.trim() !== "",
              ),
  ...
  return (
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.betaNotice}>
        ...
        {featureFlags.phase1NoContactOnly ? (
          <Text style={styles.betaNoticeSubtext}>
            Phase 1: No-Contact recovery contract flows are prioritized.
            Additional oath categories are hidden during beta hardening.
          </Text>
        ) : null}
      </View>
      ...
      >
        {streams.map((stream) => (
          <TouchableOpacity
            ...
            style={[
              styles.chip,
              selectedStream === stream && styles.chipSelected,
            ]}
            onPress={() => {
          ...
          >
            <Text
              style={[
                styles.chipText,
                selectedStream === stream && styles.chipTextSelected,
              ]}
            >
              {stream}
            </Text>
          ...
          <View style={styles.categoryGrid}>
            {streamCategories.map((cat) => (
              <TouchableOpacity
                ...
                style={[
                  styles.categoryChip,
                  oathCategory === cat.value && styles.categoryChipSelected,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.categoryText,
                    oathCategory === cat.value && styles.categoryTextSelected,
                  ]}
                >
                  {cat.label}
                </Text>
      ...
      {oathCategory.startsWith("RECOVERY_") ? (
        <>
          ...
          {oathCategory === "RECOVERY_NOCONTACT" ? (
            <>
              ...
              <Text style={styles.hint}>
                List up to 3 people/entities you are avoiding. These will be
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        setTargetIdentifiers(
                          targetIdentifiers.filter((_, i) => i !== idx),
                        )
                      }
                    >
          ...
          {[
            {
              key: "voluntary",
              label: "I am entering this contract voluntarily.",
            },
            {
              key: "noMinors",
            ...
            {
              key: "noLegalObligations",
              label: "This does not violate any legal obligations.",
            },
          ].map((ack) => (
            <TouchableOpacity
              key={ack.key}
              ...
              onPress={() =>
                setAcks({
                  ...acks,
                  [ack.key]: !acks[ack.key as keyof typeof acks],
                })
              }
            >
              <View
                style={[
                  styles.checkbox,
                  acks[ack.key as keyof typeof acks] && styles.checkboxSelected,
                ]}
              >
                {acks[ack.key as keyof typeof acks] && (
      ...
      <View style={styles.presetRow}>
        {STAKE_PRESETS.map((preset) => (
          <TouchableOpacity
            ...
            style={[
              styles.presetChip,
              Number(stakeAmount || 0) === preset.value &&
      ...
      <Text style={styles.hint}>
        Bounded range: ${MIN_STAKE_USD} to ${MAX_STAKE_USD}. Choose a commitment
        that is meaningful but sustainable.
      </Text>
      <Text style={styles.hint}>
        {featureFlags.testMoneyMode
      ...
      {hasStake ? (
        <View style={styles.lossPanel}>
          ...
          <Text style={styles.lossLine}>
            Vault hold: ${boundedStake.toFixed(2)}
          </Text>
          <Text style={styles.lossLine}>
            Per-day exposure ({durationDays}d): ${perDayExposure.toFixed(2)}
          </Text>
          <Text style={styles.lossLine}>
            Weekly loss cap policy (
            {Math.round(MAX_WEEKLY_LOSS_VELOCITY_PCT * 100)}%): $
            {weeklyLossCap.toFixed(2)}
          </Text>
        </View>
      ) : null}
      ...
      <View style={styles.durationRow}>
        {DURATION_OPTIONS.map((opt) => (
          <TouchableOpacity
            ...
            style={[
              styles.durationChip,
              durationDays === opt.value && styles.durationChipSelected,
            ]}
          ...
          >
            <Text
              style={[
                styles.durationText,
                durationDays === opt.value && styles.durationTextSelected,
              ]}
            >
              {opt.label}
            </Text>
