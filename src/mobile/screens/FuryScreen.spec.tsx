...
describe("FuryScreen – trace-ID display", () => {
  ...
  it("renders trace ID for accuracy demotion error", () => {
    const tree = SupportTraceErrorBanner({
      value:
        "Accuracy below threshold — review privileges suspended [request_id: fury-acc-low]",
    }) as any;
    const text = collectText(tree);
...
describe("FuryScreen – render tests", () => {
  ...
  beforeEach(() => {
    ...
    (ApiClient.getFuryQueue as jest.Mock).mockReturnValue(
      new Promise(() => {}),
    );
    (ApiClient.getFuryStats as jest.Mock).mockReturnValue(
      new Promise(() => {}),
    );
  });
