...
function getTxLabel(type: string): { label: string; color: string } {
  return (
    TX_TYPE_LABELS[type] || { label: type || "Transaction", color: "#888" }
  );
}
        ...
        <>
          ...
          <View style={styles.balanceCard}>
            ...
            <Text
              style={[
                styles.statusText,
                { color: balance.status === "ACTIVE" ? "#84cc16" : "#ef4444" },
              ]}
            >
              {balance.status}
            </Text>
              ...
              <View style={styles.tiersContainer}>
                {balance.allowed_tiers.map((tier) => (
                  <View
                    ...
                    style={[
                      styles.tierBadge,
                      { backgroundColor: getTierColor(tier) + "30" },
                    ]}
                  >
                    <Text
                      style={[styles.tierText, { color: getTierColor(tier) }]}
                    >
                      {tier}
                    </Text>
                  </View>
