...
describe("DigitalExhaustScreen", () => {
  ...
  it("renders scan header and privacy explanation", () => {
    const { container } = render(
      React.createElement(DigitalExhaustScreen, {
        route: mockRoute,
  ...
  it("runs local scan and submits zk proof marker", async () => {
    ...
    fireEvent.click(
      getByText("START SECURE SCAN").closest("button") as HTMLElement,
    );
    ...
    fireEvent.click(
      getByText("TRANSMIT PROOF").closest("button") as HTMLElement,
    );
    ...
    await waitFor(() => {
      expect(ApiClient.submitProof).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          mediaUri: expect.stringContaining(
            "zk://proof/contract-1/proofhash123?",
          ),
        }),
      );
      expect(alertSpy).toHaveBeenCalledWith(
        ...
        expect.stringContaining(
          "Proof proof_123 has been routed while preserving private logs.",
        ),
        expect.any(Array),
      );
    });
