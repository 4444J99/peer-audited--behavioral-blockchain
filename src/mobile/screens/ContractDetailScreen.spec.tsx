...
describe("ContractDetailScreen", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    (ApiClient.getContract as jest.Mock).mockResolvedValue(
      activeRecoveryContract,
    );
  ...
  it("routes recovery check-in into the Attestation flow", async () => {
    ...
    fireEvent.click(
      getByText("Daily Check-In").closest("button") as HTMLElement,
    );

    expect(mockNavigation.navigate).toHaveBeenCalledWith("Attestation", {
  ...
  it("routes automatic scan into DigitalExhaust with a safe label", async () => {
    ...
    fireEvent.click(
      getByText("Automatic Scan").closest("button") as HTMLElement,
    );
  ...
  it("uses a grace day and reloads contract", async () => {
    ...
    fireEvent.click(
      getByText("Use Grace Day").closest("button") as HTMLElement,
    );
