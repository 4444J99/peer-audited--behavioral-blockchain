...
jest.mock("./services/ApiClient", () => ({
  ApiClient: {
    getMobileBootstrap: jest.fn().mockResolvedValue({
      ...
      labels: {
        betaBanner: "Private beta",
        complianceNotice: "Test money only",
      },
...
describe("App enterprise SSO wiring", () => {
  beforeEach(() => {
    ...
    (EnterpriseSSO.initializeDeepLinkListener as jest.Mock).mockResolvedValue(
      undefined,
    );
  });
  ...
  it("registers the enterprise SSO deep link listener on mount", async () => {
    ...
    expect(EnterpriseSSO.initializeDeepLinkListener).toHaveBeenCalledWith(
      expect.any(Function),
    );
  });
  ...
  it("leaves the auth stack when the SSO token exchange succeeds", async () => {
    ...
    const callback = (EnterpriseSSO.initializeDeepLinkListener as jest.Mock)
      .mock.calls[0][0];
    ...
    expect(nativeScreenRegistry.map((route) => route.name)).not.toContain(
      "Login",
    );
  });
  ...
  it("stays in the auth stack when the SSO token exchange fails", async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "Enterprise SSO handoff failed: Invalid enterprise token",
      ),
    );
  ...
  it("reports a listener that cannot be initialized instead of leaving an unhandled rejection", async () => {
    ...
    (
      EnterpriseSSO.initializeDeepLinkListener as jest.Mock
    ).mockRejectedValueOnce(new Error("Linking unavailable"));

    await mountApp();

    expect(error).toHaveBeenCalledWith(
      expect.stringContaining(
        "Enterprise SSO listener failed to initialize: Linking unavailable",
      ),
    );
    error.mockRestore();
  });
