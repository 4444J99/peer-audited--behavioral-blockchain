...
describe("deep link configuration", () => {
  ...
  it("maps nested contracts screens", () => {
    ...
    expect(screens.Contracts.screens.ContractDetail).toBe(
      "contracts/:contractId",
    );
    expect(screens.Contracts.screens.Attestation).toBe(
      "contracts/:contractId/attest",
    );
...
describe("isEnterpriseSSOLink", () => {
  it("matches the styx:// enterprise SSO entry point", () => {
    expect(isEnterpriseSSOLink("styx://enterprise/callback?token=abc")).toBe(
      true,
    ); // allow-secret
  });
  ...
  it("matches the bundle-id scheme too", () => {
    expect(
      isEnterpriseSSOLink(
        "com.styxprotocol.app://enterprise/callback?token=abc",
      ),
    ).toBe(true); // allow-secret
  });
  ...
  it("does not match foreign schemes", () => {
    expect(
      isEnterpriseSSOLink("https://example.com/enterprise/?token=abc"),
    ).toBe(false); // allow-secret
  });
...
describe("resolveNotificationDeepLink", () => {
  ...
  it("returns null for attestation_reminder without contractId", () => {
    expect(
      resolveNotificationDeepLink({ type: "attestation_reminder" }),
    ).toBeNull();
  });
  ...
  it("returns null for grace_day_reminder without contractId", () => {
    expect(
      resolveNotificationDeepLink({ type: "grace_day_reminder" }),
    ).toBeNull();
  });
  ...
  it("returns null for deadline_warning without contractId", () => {
    expect(
      resolveNotificationDeepLink({ type: "deadline_warning" }),
    ).toBeNull();
  });

  it("resolves fury_assignment to fury screen", () => {
    expect(resolveNotificationDeepLink({ type: "fury_assignment" })).toBe(
      "styx://fury",
    );
  });
  ...
  it("resolves wallet_update to wallet screen", () => {
    expect(resolveNotificationDeepLink({ type: "wallet_update" })).toBe(
      "styx://wallet",
    );
  });
});
