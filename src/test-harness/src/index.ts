  ...
  .action(async (options) => {
    ...
    results.forEach((suite) => {
      ...
      suite.results.forEach((res) => {
        const icon =
          res.status === "PASS" ? "✅" : res.status === "SKIP" ? "⏭️" : "❌";
        console.log(
          `${icon} [${res.check}] ${res.status}${res.message ? `: ${res.message}` : ""}`,
        );
      });
    });

    const hasFailures = results.some((suite) =>
      suite.results.some((res) => res.status === "FAIL"),
    );
    if (hasFailures) {
      console.log("\n❌ Audit failed.");
      process.exit(1);
