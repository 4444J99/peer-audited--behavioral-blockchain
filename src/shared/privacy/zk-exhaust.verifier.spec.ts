...
describe("ZKExhaustVerifier", () => {
  ...
  it("binds the signature to the verifier target (cannot be lifted onto another claim)", () => {
    ...
    const tampered = {
      ...proof,
      senderPseudonym: ZKExhaustVerifier.pseudonymForPhone("555-0000"),
    };
    expect(ZKExhaustVerifier.verify(tampered, tampered.senderPseudonym)).toBe(
      false,
    );
  });
  ...
  it("enforces the verifier challenge when supplied (replay across sessions fails)", () => {
    ...
    expect(
      ZKExhaustVerifier.verify(proof, proof.senderPseudonym, challenge),
    ).toBe(true);
    expect(
  ...
  it("rejects a stale proof beyond the max-age window", () => {
    ...
    const stale = {
      ...proof,
      timestamp: new Date(
        Date.now() - ZK_PROOF_MAX_AGE_MS - 1000,
      ).toISOString(),
    };
    ...
    expect(ZKExhaustVerifier.verify(restale, restale.senderPseudonym)).toBe(
      true,
    );
  });
  ...
  it("throws in production when ZK_EXHAUST_SECRET is unset", () => {
    ...
    expect(() => ZKExhaustVerifier.generateProof("Hey baby", PHONE)).toThrow(
      /ZK_EXHAUST_SECRET/,
    );
  });
});
