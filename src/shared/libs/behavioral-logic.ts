...
export function validateGatewayOath(params: {
  ...
  if (params.totalContracts > 0) {
    return {
      allowed: false,
      reason: "Gateway Oath available for first contract only",
    };
  }
  if (params.requestedStakeCents < GATEWAY_OATH_MIN_STAKE_CENTS) {
    return {
      allowed: false,
      reason: `Minimum gateway stake is $${(GATEWAY_OATH_MIN_STAKE_CENTS / 100).toFixed(2)}`,
    };
  }
  if (params.requestedStakeCents > GATEWAY_OATH_MAX_STAKE_CENTS) {
    return {
      allowed: false,
      reason: `Maximum gateway stake is $${(GATEWAY_OATH_MAX_STAKE_CENTS / 100).toFixed(2)}`,
    };
  }
  if (params.requestedDurationDays < GATEWAY_OATH_MIN_DURATION_DAYS) {
    return {
      allowed: false,
      reason: `Minimum gateway duration is ${GATEWAY_OATH_MIN_DURATION_DAYS} days`,
    };
  }
  if (params.requestedDurationDays > GATEWAY_OATH_MAX_DURATION_DAYS) {
    return {
      allowed: false,
      reason: `Maximum gateway duration is ${GATEWAY_OATH_MAX_DURATION_DAYS} days`,
    };
  }
  return { allowed: true };
}
...
export function calculateStakeTaper(
  ...
  for (let w = 0; w < completedWeeks; w++) {
    ...
    currentStake = Math.max(
      currentStake - reduction,
      STAKE_TAPER_MIN_FLOOR_CENTS,
    );
    weekNum++;
  }
  return {
...
export const CM_REWARD_TIERS: Array<{
  startDay: number;
  endDay: number;
  rewardCents: number;
}> = [
...
export function calculateMicroReward(): {
  base: number;
  bonus: number;
  total: number;
} {
  const base =
    MICRO_REWARD_RANGE_CENTS[0] +
    Math.floor(
      Math.random() *
        (MICRO_REWARD_RANGE_CENTS[1] - MICRO_REWARD_RANGE_CENTS[0] + 1),
    );
...
export function calculateHabitStrength(
  ...
  return (
    ...
    (1 +
      Math.exp(
        -HABIT_STRENGTH_CURVE_STEEPNESS *
          (completedProofs - HABIT_STRENGTH_AUTOMATICITY_MIDPOINT),
      ))
  );
}
...
export const BBO_LIBRARY: BboEntry[] = [
  {
    ...
    category: "social",
    title: "Call a Friend",
    description: "Phone a trusted friend for a 10-minute catch-up",
    durationMinutes: 10,
  ...
  },
  {
    ...
    category: "physical",
    title: "Go for a Walk",
    description: "Step outside for a 15-minute walk without your phone",
    durationMinutes: 15,
  ...
  },
  {
    ...
    title: "3-Minute Breath",
    description: "Box breathing: 4-4-4-4 pattern for 3 minutes",
    durationMinutes: 3,
  ...
  },
  {
    ...
    category: "reflection",
    title: "Brain Dump",
    description: "Write whatever comes to mind for 5 minutes",
    durationMinutes: 5,
  ...
  },
  {
    ...
    category: "physical",
    title: "Quick Workout",
    description: "20 pushups, 20 squats, 30-second plank",
    durationMinutes: 5,
    tags: ["physical", "endorphins"],
  },
  {
    id: "bbo-music",
    category: "creative",
    title: "Play Music",
    description: "Play one song you love on an instrument or speaker",
    durationMinutes: 4,
    tags: ["creative", "mood"],
  },
  {
    ...
    category: "focus",
    title: "Read 5 Pages",
    description: "Read 5 pages of any book you are currently reading",
    durationMinutes: 10,
    tags: ["focus", "learning"],
  },
  {
  ...
  },
  {
    ...
    description: "Put on a 10-minute episode of a favorite podcast",
    durationMinutes: 10,
  ...
  },
  {
    ...
    description: "5-minute full-body stretching routine",
    durationMinutes: 5,
...
export function getBboRecommendations(category: string, count = 3): BboEntry[] {
  ...
  const shuffled = [...(byCategory.length > 0 ? byCategory : BBO_LIBRARY)].sort(
    () => Math.random() - 0.5,
  );
...
export const FRICTION_AUDIT_QUESTIONS = [
  {
    id: "good_habit_steps",
    question:
      "How many steps between you and your desired habit? (1 = immediate, 5 = many steps)",
    weight: 3,
  },
  {
    id: "bad_habit_access",
    question:
      "How easy is it to access the behavior you want to avoid? (1 = very hard, 5 = very easy)",
    weight: 3,
  },
  {
    id: "environment_triggers",
    question:
      "How many environmental triggers remind you of the old behavior? (1 = none, 5 = many)",
    weight: 2,
  },
  {
    id: "social_support",
    question:
      "Do people around you support this change? (1 = strongly, 5 = actively oppose)",
    weight: 2,
  },
  {
    id: "time_availability",
    question:
      "Do you have dedicated time for this habit? (1 = always, 5 = never)",
    weight: 1,
  },
];
...
export function calculateFrictionScore(
  ...
  if (answers.good_habit_steps && answers.good_habit_steps >= 3) {
    recommendations.push(
      "Reduce steps to your habit: prepare equipment/tools the night before.",
    );
  }
  if (answers.bad_habit_access && answers.bad_habit_access >= 3) {
    recommendations.push(
      "Increase friction for the behavior you want to avoid: block apps, remove triggers.",
    );
  }
  if (answers.environment_triggers && answers.environment_triggers >= 3) {
    recommendations.push(
      "Remove environmental triggers: rearrange your space, change your route.",
    );
  }
  if (answers.social_support && answers.social_support >= 3) {
    recommendations.push(
      "Find social support: tell a friend about your commitment, join a pod.",
    );
  }
...
export const TEMPTATION_BUNDLE_TEMPLATES: TemptationBundle[] = [
  {
    id: "tb-exercise-netflix",
    needBehavior: "exercise",
    wantBehavior: "watch Netflix",
    category: "BIOLOGICAL",
  },
  {
    id: "tb-study-coffee",
    needBehavior: "study",
    wantBehavior: "drink specialty coffee",
    category: "COGNITIVE",
  },
  {
    id: "tb-work-music",
    needBehavior: "deep work",
    wantBehavior: "listen to music",
    category: "PROFESSIONAL",
  },
  {
    id: "tb-write-treat",
    needBehavior: "write",
    wantBehavior: "eat a treat",
    category: "CREATIVE",
  },
  {
    id: "tb-clean-podcast",
    needBehavior: "clean",
    wantBehavior: "listen to podcast",
    category: "ENVIRONMENTAL",
  },
  {
    id: "tb-meditate-walk",
    needBehavior: "meditate",
    wantBehavior: "go for a walk after",
    category: "RECOVERY",
  },
];
...
export function classifyAbandonment(params: {
  ...
  if (
    ...
    return {
      type: AbandonmentType.HAPPY_GRADUATE,
      confidence: 0.8,
      signal: "high completion rate + high integrity + inactive",
    };
  }
  if (
    failedContracts >= 2 &&
    ...
    return {
      type: AbandonmentType.FRUSTRATED,
      confidence: 0.75,
      signal: "multiple failures + support tickets",
    };
  }
  if (
    ...
    return {
      type: AbandonmentType.BORED,
      confidence: 0.7,
      signal: "multiple completions + low exit streak",
    };
  }
  if (daysSinceLastActive > 30 && integrityScore > 50) {
    return {
      type: AbandonmentType.LIFE_EVENT,
      confidence: 0.6,
      signal: "long absence + moderate integrity",
    };
  }
  return {
    type: AbandonmentType.FRUSTRATED,
    confidence: 0.5,
    signal: "default classification",
  };
}
...
export function checkReentryEligibility(params: {
  ...
  const reducedStake = Math.round(
    params.previousStakeCents * REENTRY_STAKE_DISCOUNT_PCT,
  );
  return {
    eligible: true,
...
export const EXIT_INTERVIEW_QUESTIONS_SUCCESS = [
  ...
  {
    id: "difficulty",
    question: "How difficult was this contract for you?",
    type: "rating_1_5",
  },
  ...
  {
    id: "nps",
    question: "How likely are you to recommend Styx to a friend?",
    type: "rating_0_10",
  },
];
...
export const EXIT_INTERVIEW_QUESTIONS_FAILURE = [
  {
    id: "satisfaction",
    question: "How satisfied are you with how this was handled?",
    type: "rating_1_5",
  },
  { id: "why_failed", question: "What caused the failure?", type: "text" },
  {
    id: "what_would_help",
    question: "What would have helped you succeed?",
    type: "text",
  },
  {
    id: "return_intention",
...
export function getRecoveryState(
  ...
  if (daysSinceContractStart <= 30 && isWeekend)
    return RecoveryState.WEEKEND_SHIELD;
  if (daysSinceContractStart === 21) return RecoveryState.REWARD_INJECTION;
  if (daysSinceContractStart >= 45 && daysSinceContractStart <= 60)
    return RecoveryState.FRICTION_DELAY;
...
export function calculateDisenchantment(
  ratings: DisenchantmentEntry[],
  ...
  const dropPct =
    startScore > 0
      ? Math.round(((startScore - currentScore) / startScore) * 100)
      : 0;
  const firstHalf = sorted.slice(0, Math.floor(sorted.length / 2));
  const secondHalf = sorted.slice(Math.floor(sorted.length / 2));
  const firstAvg =
    firstHalf.reduce((s, e) => s + e.rating, 0) / firstHalf.length;
  const secondAvg =
    secondHalf.reduce((s, e) => s + e.rating, 0) / secondHalf.length;
...
const DISCONTINUITY_OATH_MAP: Record<LifeTransitionType, string[]> = {
  [LifeTransitionType.MOVE]: [
    "BIOLOGICAL_CARDIO",
    "ENVIRONMENTAL_TIDINESS",
    "COGNITIVE_DIGITAL",
  ],
  [LifeTransitionType.NEW_JOB]: [
    "PROFESSIONAL_TIME",
    "COGNITIVE_FOCUS",
    "BIOLOGICAL_SLEEP",
  ],
  [LifeTransitionType.BREAKUP]: [
    "RECOVERY_NOCONTACT",
    "BIOLOGICAL_SOBRIETY",
    "CREATIVE_WRITING",
  ],
  [LifeTransitionType.GRADUATION]: [
    "COGNITIVE_LEARNING",
    "PROFESSIONAL_CODE",
    "BIOLOGICAL_WEIGHT",
  ],
  [LifeTransitionType.NEW_PARENT]: [
    "BIOLOGICAL_SLEEP",
    "RECOVERY_NOCONTACT",
    "ENVIRONMENTAL_TIDINESS",
  ],
  [LifeTransitionType.RETIREMENT]: [
    "CREATIVE_WRITING",
    "BIOLOGICAL_CARDIO",
    "COGNITIVE_LEARNING",
  ],
  [LifeTransitionType.OTHER]: [
    "BIOLOGICAL_WEIGHT",
    "COGNITIVE_FOCUS",
    "RECOVERY_NOCONTACT",
  ],
...
export function detectDiscontinuityWindow(params: {
  ...
  const remaining = Math.max(
    0,
    DISCONTINUITY_WINDOW_DAYS - params.daysSinceTransition,
  );
  const discountPct = active
    ? Math.max(
        10,
        50 - Math.floor((remaining / DISCONTINUITY_WINDOW_DAYS) * 40),
      )
    : 0;
  return {
    active,
    ...
    suggestedOaths:
      DISCONTINUITY_OATH_MAP[params.transitionType] ||
      DISCONTINUITY_OATH_MAP[LifeTransitionType.OTHER],
  };
}
...
export function validateImplementationIntention(
  intention: ImplementationIntention,
): ImplementationIntentionValidation {
  const errors: string[] = [];
  if (!intention.behavior || intention.behavior.length < 2)
    errors.push("Behavior must be at least 2 characters");
  if (!IMPLEMENTATION_INTENTION_TIME_FORMAT.test(intention.time))
    errors.push("Time must be in HH:MM 24-hour format");
  if (!intention.location || intention.location.length < 2)
    errors.push("Location must be at least 2 characters");
  return { valid: errors.length === 0, errors };
}

export function generateImplementationIntentionTemplate(
  category: OathCategory,
): string {
  const templates: Record<string, string> = {
    [OathCategory.CARDIOVASCULAR_STAMINA]:
      "I will exercise at 07:00 in the living room",
    [OathCategory.DEEP_WORK_FOCUS]:
      "I will do deep work at 09:00 in my home office",
    [OathCategory.SLEEP_INTEGRITY]:
      "I will start my bedtime routine at 22:00 in my bedroom",
    [OathCategory.DIGITAL_FASTING]:
      "I will put my phone away at 20:00 in the kitchen drawer",
    [OathCategory.SOBRIETY_HRV]:
      "I will attend my meeting at 19:00 at the community center",
...
export const PASSIVE_PROVIDER_CONFIG: Record<
  PassiveProvider,
  { label: string; proofType: string; refreshHrs: number }
> = {
  [PassiveProvider.STRAVA]: {
    label: "Strava",
    proofType: "workout_completed",
    refreshHrs: 1,
  },
  [PassiveProvider.DUOLINGO]: {
    label: "Duolingo",
    proofType: "lesson_completed",
    refreshHrs: 1,
  },
  [PassiveProvider.GITHUB]: {
    label: "GitHub",
    proofType: "commit_pushed",
    refreshHrs: 6,
  },
  [PassiveProvider.CALM]: {
    label: "Calm",
    proofType: "meditation_completed",
    refreshHrs: 2,
  },
  [PassiveProvider.KINDLE]: {
    label: "Kindle",
    proofType: "reading_sessions",
    refreshHrs: 4,
  },
};
...
export function evaluatePodBroadcast(params: {
  ...
  const cooldownHrs = Math.min(
    24,
    Math.max(1, Math.floor(params.failureCount / params.memberCount) * 4),
  );
  ...
  return {
    ...
    broadcastCooldownHrs: cooldownHrs,
    dampened,
  };
}
...
export function assessAuditorWellness(params: {
  ...
  const fatigueScore = Math.min(
    consecutiveScore + speedScore + rejectionScore,
    100,
  );
  ...
  return {
    ...
    consecutiveReviews: params.consecutiveReviews,
    fatigueScore,
    biasRisk,
    recommendedBreak: fatigueScore > 60,
  };
}
...
export function calculateGenerosityGrant(
  giverCompletedContracts: number,
  giverAverageStakeCents: number,
): { maxDonationCents: number; badgeThreshold: number } {
...
export const ACADEMY_CURRICULUM: AcademyModule[] = [
  {
    ...
    description: "The cue-routine-reward loop and why willpower fails",
    durationMin: 10,
    rewardCents: 25,
  },
  {
    ...
    description: "Design your environment for automatic good choices",
    durationMin: 15,
    rewardCents: 35,
  },
  {
    ...
    description: "Start so small you cannot say no",
    durationMin: 8,
    rewardCents: 25,
  },
  {
    ...
    description: "I am the person who... — rewrite your self-concept",
    durationMin: 12,
    rewardCents: 35,
  },
  {
    ...
    description: "Link want behaviors with need behaviors",
    durationMin: 10,
    rewardCents: 25,
  },
  {
    ...
    description: "I will [X] at [TIME] in [LOCATION]",
    durationMin: 10,
    rewardCents: 25,
  },
  {
    ...
    description: "Make it obvious, attractive, easy, satisfying",
    durationMin: 20,
    rewardCents: 50,
  },
  {
    id: "AM-08",
    title: "Recovery & Disenchantment",
    description: "Why the old habit loses its grip",
    durationMin: 15,
    rewardCents: 35,
  },
];

export function getAcademyProgress(completedIds: string[]): {
  completed: number;
  total: number;
  pct: number;
  ...
  const completed = completedIds.filter((id) =>
    ACADEMY_CURRICULUM.some((m) => m.id === id),
  ).length;
  const unlockedRewards = ACADEMY_CURRICULUM.filter((m) =>
    completedIds.includes(m.id),
  ).reduce((sum, m) => sum + m.rewardCents, 0);
  return {
    completed,
    total,
...
const RESISTANCE_KEYWORDS: Record<ResistancePattern, RegExp[]> = {
  [ResistancePattern.DISPLACEMENT]: [
    /not my fault/i,
    /they made me/i,
    /because of/i,
    /would have if/i,
  ],
  [ResistancePattern.SELF_DRAMATIZATION]: [
    /always fail/i,
    /never work/i,
    /worst ever/i,
    /impossible/i,
  ],
  [ResistancePattern.VICTIM_NARRATIVE]: [
    /why me/i,
    /unfair/i,
    /no one understands/i,
    /against me/i,
  ],
  [ResistancePattern.MINIMIZATION]: [
    /it was just/i,
    /only once/i,
    /barely counts/i,
    /not that bad/i,
  ],
  [ResistancePattern.INTELLECTUALIZATION]: [
    /technically/i,
    /theoretically/i,
    /research says/i,
    /but actually/i,
...
export function detectResistancePatterns(text: string): ResistanceSignal[] {
  ...
  for (const [pattern, regexps] of Object.entries(RESISTANCE_KEYWORDS)) {
    for (const re of regexps) {
      ...
      if (match) {
        signals.push({
          pattern: pattern as ResistancePattern,
          confidence: 0.6,
          snippet: match[0],
        });
        break;
...
export const INTAKE_ASSESSMENT: AssessmentQuestion[] = [
  {
    id: "AS-01",
    question: "I follow through on commitments even when I do not feel like it",
    scale: "agree_1_5",
    dimension: "CONSCIENTIOUSNESS",
  },
  {
    id: "AS-02",
    question: "I act on impulse without thinking about consequences",
    scale: "agree_1_5",
    dimension: "IMPULSIVITY",
  },
  {
    id: "AS-03",
    question: "I am confident I can change my habits",
    scale: "agree_1_5",
    dimension: "SELF_EFFICACY",
  },
  {
    id: "AS-04",
    question: "How often do you feel motivated to work on your goals?",
    scale: "frequency_1_5",
    dimension: "MOTIVATION",
  },
  {
    id: "AS-05",
    question: "I have people in my life who support my growth",
    scale: "agree_1_5",
    dimension: "SOCIAL_SUPPORT",
  },
  {
    id: "AS-06",
    question: "I complete tasks I start",
    scale: "agree_1_5",
    dimension: "CONSCIENTIOUSNESS",
  },
  {
    id: "AS-07",
    question: "How important is personal growth to you right now?",
    scale: "importance_1_5",
    dimension: "MOTIVATION",
  },
  {
    id: "AS-08",
    question: "I can resist temptation when I set my mind to it",
    scale: "agree_1_5",
    dimension: "IMPULSIVITY",
  },
];
...
export interface AssessmentProfile {
  ...
  archetype:
    "ACHIEVER" | "STRUGGLER" | "SOCIAL_DEPENDENT" | "IMPULSIVE" | "BALANCED";
}
...
export function calculateAssessmentProfile(
  ...
  const avg = (vals: number[]) =>
    vals.length > 0 ? vals.reduce((a, b) => a + b, 0) / vals.length : 3;
...
export async function createDecoProof(
  params: DecoProofRequest,
): Promise<DecoProofResult> {
  const timestamp = new Date().toISOString();
  const payload =
    params.url + params.selector + params.expectedValue + timestamp;
  ...
  return {
    verified: true,
    revealedFields: [params.selector],
    timestamp,
    commitmentHash,
  };
}
...
export function resolveOracleFailure(
  arbiterVerdicts: ArbiterVerdict[],
): ArbiterVerdict {
  const upheld = arbiterVerdicts.filter((v) => v.verdict === "UPHELD").length;
  const overturned = arbiterVerdicts.filter(
    (v) => v.verdict === "OVERTURNED",
  ).length;
  const majority: "UPHELD" | "OVERTURNED" =
    upheld >= overturned ? "UPHELD" : "OVERTURNED";
  return {
    arbiterId: "consensus",
    contractId: arbiterVerdicts[0]?.contractId ?? "",
    verdict: majority,
    stakeAtRiskCents: Math.max(
      ...arbiterVerdicts.map((v) => v.stakeAtRiskCents),
    ),
  };
}
...
export function quoteStablecoinStake(
  usdCents: number,
  stablecoinType: "USDC" | "USDT",
): StablecoinQuote {
  ...
  return {
    usdCents,
    stablecoinAmount: ((usdCents / 100) * rate).toFixed(6),
    stablecoinType,
    exchangeRate: rate,
  };
}
...
export function calculateRevenueShare(params: {
  ...
  return {
    userShareCents: Math.round(params.totalPoolCents * sharePct),
    sharePct: Math.round(sharePct * 10000) / 100,
  };
}
...
export const WHISTLEBLOWER_MULTIPLIER = {
  FRAUD: 3,
  ABUSE: 2,
  COLLUSION: 1.5,
  OTHER: 1,
...
export function createWhistleblowerReport(
  category: WhistleblowerReport["category"],
): {
  rewardCents: number;
  anonymityLevel: WhistleblowerReport["anonymityLevel"];
  escalationPath: string;
} {
  const rewardCents = Math.round(
    WHISTLEBLOWER_BASE_REWARD * (WHISTLEBLOWER_MULTIPLIER[category] ?? 1),
  );
  return {
    rewardCents,
    anonymityLevel: "FULL",
    escalationPath: `${category.toLowerCase()}-review-panel`,
  };
}
