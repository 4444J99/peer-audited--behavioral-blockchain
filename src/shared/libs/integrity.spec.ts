...
describe("calculateIntegrity", () => {
  ...
  it("should floor the score at 0", () => {
    const score = calculateIntegrity(
      makeUser({ fraudStrikes: 10, failedOaths: 10 }),
    );
    expect(score).toBe(0);
  });
  ...
  it("should keep fraud penalties meaningful at high scores", () => {
    ...
    const withFraud = calculateIntegrity(
      makeUser({ completedOaths: 200, fraudStrikes: 1 }),
    );
...
describe("getAllowedTiers", () => {
  ...
  it("should return MICRO + STANDARD for score 50-99", () => {
    expect(getAllowedTiers(50)).toEqual([
      "TIER_1_MICRO_STAKES",
      "TIER_2_STANDARD",
    ]);
    expect(getAllowedTiers(99)).toEqual([
      "TIER_1_MICRO_STAKES",
      "TIER_2_STANDARD",
    ]);
  });
...
describe("getTierMaxStake", () => {
  ...
  it("should return 10000 cents for STANDARD tier", () => {
    expect(getTierMaxStake(["TIER_1_MICRO_STAKES", "TIER_2_STANDARD"])).toBe(
      10000,
    );
  });
