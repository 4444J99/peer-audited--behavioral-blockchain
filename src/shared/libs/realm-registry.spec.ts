...
describe("RealmRegistry", () => {
  ...
  describe("getRealmForCategory", () => {
    ...
    it("should return undefined for unknown category values", () => {
      expect(
        getRealmForCategory("UNKNOWN_THING" as OathCategory),
      ).toBeUndefined();
    });
  });
  ...
  describe("getOathCategoriesForRealm", () => {
    ...
    it("should return 4 categories for COGNITIVE_DEVICE", () => {
      expect(getOathCategoriesForRealm(RealmId.COGNITIVE_DEVICE)).toHaveLength(
        4,
      );
    });

    it("should return 3 categories for PROFESSIONAL_API", () => {
      expect(getOathCategoriesForRealm(RealmId.PROFESSIONAL_API)).toHaveLength(
        3,
      );
    });

    it("should return 4 categories for CREATIVE_PROCESS", () => {
      expect(getOathCategoriesForRealm(RealmId.CREATIVE_PROCESS)).toHaveLength(
        4,
      );
    });
    ...
    it("should return 4 categories for ENVIRONMENTAL_VISUAL", () => {
      expect(
        getOathCategoriesForRealm(RealmId.ENVIRONMENTAL_VISUAL),
      ).toHaveLength(4);
    });

    it("should return 3 categories for CHARACTER_SOCIAL", () => {
      expect(getOathCategoriesForRealm(RealmId.CHARACTER_SOCIAL)).toHaveLength(
        3,
      );
    });
    ...
    it("should return 4 categories for RECOVERY_ABSTINENCE", () => {
      expect(
        getOathCategoriesForRealm(RealmId.RECOVERY_ABSTINENCE),
      ).toHaveLength(4);
    });
  ...
  describe("realm bridges", () => {
    ...
    it("should have no bridges for non-recovery realms in Phase 1", () => {
      const nonRecovery = REALM_REGISTRY.filter(
        (r) => r.id !== RealmId.RECOVERY_ABSTINENCE,
      );
