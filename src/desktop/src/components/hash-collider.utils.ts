...
export function getCollisionSimilarity(collision: HashCollision): number {
  return clampSimilarity(
    collision.duplicate?.similarity ?? collision.origin?.similarity ?? 0,
  );
}

export function classifyCollisionSeverity(
  similarity: number,
): CollisionSeverity {
...
export function normalizeCollisionList(
  collisions: HashCollision[],
): HashCollision[] {
  return [...collisions].sort(
    (a, b) => getCollisionSimilarity(b) - getCollisionSimilarity(a),
  );
}
...
export function filterCollisionsByThreshold(
  ...
  return collisions.filter(
    (collision) => getCollisionSimilarity(collision) >= normalizedThreshold,
  );
}
...
export function buildCollisionTicketDraft(collision: HashCollision): string {
  ...
  const severity = classifyCollisionSeverity(
    getCollisionSimilarity(collision),
  ).toUpperCase();
