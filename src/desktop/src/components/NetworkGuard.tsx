...
export default function NetworkGuard({
  ...
  const [status, setStatus] = useState<"CHECKING" | "SECURE" | "BLOCKED">(
    "CHECKING",
  );
  ...
  return (
    <div className="network-guard-container">
      <div
        className={`network-guard-card ${status === "BLOCKED" ? "blocked" : "secure"}`}
      >
        <div className="network-guard-header">
          ...
          <h1
            className={`network-guard-title ${status === "BLOCKED" ? "blocked" : ""}`}
          >
        ...
        <div className="logs-container">
          {logs.map((log, i) => (
            <div
              key={i}
              className={`log-item ${log.includes("Validating") ? "validating" : ""}`}
            >
              {log}
            </div>
          ))}
        ...
        {status === "BLOCKED" && (
          <div className="blocked-alert">
            ACCESS DENIED: Your current IP address is not recognized on the
            corporate VPN ingress list. The Judge terminal will not load outside
            a secure tunnel.
          </div>
        )}
      </div>
