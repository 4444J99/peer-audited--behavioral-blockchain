...
const furies: Fury[] = [];
for (let i = 0; i < 70; i++)
  furies.push({ id: `H_${i}`, type: "HONEST", stake: 100, accuracyScore: 1.0 });
for (let i = 0; i < 20; i++)
  furies.push({ id: `R_${i}`, type: "RANDOM", stake: 100, accuracyScore: 1.0 });
for (let i = 0; i < 10; i++)
  furies.push({
    id: `C_${i}`,
    type: "COLLUDING",
...
console.log(
  `[INIT] Total Furies: ${furies.length} (70 Honest, 20 Random, 10 Colluding)`,
);
...
for (let round = 1; round <= 1000; round++) {
  ...
  const availablePool = furies.filter(
    (f) => f.stake > 0 && f.accuracyScore > 0.5,
  );
...
console.log(
  `[RESULTS] Honest Consensus Rate: ${((honestConsensusMatches / 1000) * 100).toFixed(2)}%`,
);
console.log(`[RESULTS] Honey Traps Evaluated: ${honeyTrapsCaught}`);

const bankruptColluders = furies.filter(
  (f) => f.type === "COLLUDING" && f.stake <= 0,
).length;
const demotedRandoms = furies.filter(
  (f) => f.type === "RANDOM" && f.accuracyScore < 0.8,
).length;

console.log(
  `[RESULTS] Colluders Bankrupted/Slashed: ${
    furies
      .filter((f) => f.type === "COLLUDING")
      .map((f) => f.stake)
      .reduce((a, b) => a + b, 0) / 10
  }% average stake remaining`,
);
console.log(`[RESULTS] Random Guessers Demoted: ${demotedRandoms}/20`);

if (honestConsensusMatches / 1000 > 0.9) {
  console.log(
