...
const FORBIDDEN_PATTERNS: ForbiddenPattern[] = [
  ...
  {
    pattern: /dev-mock-jwt-token-alpha-omega/g,
    label: "DEV_MOCK_TOKEN",
    severity: "error",
  },
  {
    pattern: /d0000000-0000-0000-0000-000000000001/g,
    label: "DEV_MOCK_USER_ID",
    severity: "error",
  },

  // Debug backdoors
  {
    pattern: /NODE_ENV\s*!==\s*['"]production['"]\s*&&\s*token\s*===/,
    label: "DEV_TOKEN_BYPASS",
    severity: "error",
  },

  // Hardcoded default secrets — these must never be a runtime fallback.
  {
    pattern: /styx-dev-secret-do-not-use-in-production/g,
    label: "HARDCODED_JWT_SECRET",
    severity: "error",
  },
  {
    pattern: /styx-webhook-dev-secret/g,
    label: "HARDCODED_WEBHOOK_SECRET",
    severity: "error",
  },
  {
    pattern: /styx-default-secret/g,
    label: "HARDCODED_APP_SECRET",
    severity: "error",
  },
  {
    pattern: /styx-anon-salt-v1/g,
    label: "HARDCODED_ANON_SALT",
    severity: "error",
  },
  {
    pattern: /do-not-use-in-production/g,
    label: "HARDCODED_DEV_SECRET",
    severity: "error",
  },

  // Known test-only secrets — warn but don't fail (env fallbacks are acceptable)
  {
    pattern: /sk_test_mock_key/g,
    label: "STRIPE_TEST_KEY (env fallback)",
    severity: "warn",
  },
];
...
function runSecurityInvariantCheck() {
  ...
  const sourceFiles = existingGuardsDirs.flatMap((d) =>
    collectFiles(d, SOURCE_SCAN_EXTENSIONS),
  );
  for (const file of sourceFiles) {
    ...
    try {
      const content = readFileSync(file, "utf-8");
      if (
        /DEV_MOCK_TOKEN/.test(content) ||
        /dev-mock-jwt-token/.test(content)
      ) {
  ...
  console.log(
    `Scanned ${filesScanned} files across ${SCAN_DIRS.length} build directories + source guards.`,
  );
